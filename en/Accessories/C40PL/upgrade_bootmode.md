...
The C40PL is installed with the Linux operating system by default. If users want to run other operating systems, they need to use the corresponding firmware to program to the mainboard.

C40PL has a flexible startup mode. Generally, the C40PL development board will not turn brick unless the hardware is damaged.
...
## Upgrade method
C40PL supports firmware update through the following two methods:
...
C40PL loads the system from the following memory:
