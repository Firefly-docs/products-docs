...
CQ38W-3576 use `UART10` for `RS485`, which is `/dev/ttyS10` in system. And `UART8` for `RS232`, which is `/dev/ttyS8` in system.Using `UART5` for common uart,which is `/dev/ttyS5` in system.
