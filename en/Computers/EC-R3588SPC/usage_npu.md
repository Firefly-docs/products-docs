...
The RKNN model can run directly on the RK3588 platform. There are demos under `rknpu2/examples`. Refer to the `README.md` to compile Android or Linux Demo (Need cross-compile environment). You can also just download compiled [demo](https://community.t-firefly.com/en/doc/download/182).

Run yolov5 demo on the EC-R3588SPC as follows:
```shell
...
#### Run on EC-R3588SPC NPU connected to the PC

RKNN Toolkit2 runs on the PC and connects to the EC-R3588SPC through the PC's USB. RKNN Toolkit2 transfers the RKNN model to the NPU device of EC-R3588SPC to run, and then obtains the inference results, performance information, etc. from the EC-R3588SPC


* First prepare EC-R3588SPC environment: update librknnrt.so and run rknn_server
