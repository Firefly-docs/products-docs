...
After obtaining the installation package, transfer it to EC-A1684JD4 and run the following command to install it:
...
After obtaining the model file, transfer it to EC-A1684JD4 and run the following command to import it:
...
Transfer the deployment package to the EC-A1684JD4 machine, using the `scp` command is recommended:

1. Run the `ifconfig` command on EC-A1684JD4 to view the IP address.
2. Run the `scp` command to transmit the deployment package, for example: `scp llama3.tar linar@ <aibox-ip-addr>:~/`, where <aibox-ip-addr> is the IP address of EC-A1684JD4.
