...
A portion of the memory of AIBOX-1684X is allocated to NPU, VPP, and VPU devices. So the memory data obtained using commands such as `free -h` is inconsistent with the actual memory of AIBOX-1684X.
...
Obtain the memory allocation tool package from [Download Center] and transfer it to AIBOX-1684X. Run the following command to decompress the package:
