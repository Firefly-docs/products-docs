...
After obtaining the installation package, transfer it to EC-A1684JD4 FD and run the following command to install it:
...
After obtaining the model file, transfer it to EC-A1684JD4 FD and run the following command to import it:
...
Transfer the deployment package to the EC-A1684JD4 FD machine, using the `scp` command is recommended:

1. Run the `ifconfig` command on EC-A1684JD4 FD to view the IP address.
2. Run the `scp` command to transmit the deployment package, for example: `scp llama3.tar linar@ <aibox-ip-addr>:~/`, where <aibox-ip-addr> is the IP address of EC-A1684JD4 FD.
...
[Download Center]: https://community.t-firefly.com/en/doc/download/212