...
[Firefly_Linux_SDK Source]: https://en.t-firefly.com/doc/download/77.html#other_144
...
The configuration file `roc-rk3328-pc-ubuntu.mk`:
...
**Note :** `roc-rk3328-pc-ubuntu.mk` is configuration file after compiled ubuntu firmware. At the same time, users can also generate new configuration files by referring to this configuration to adapt the firmware they need.
...
#Kernel dts
export RK_KERNEL_DTS=firefly_roc-rk3328-pc_defconfig                     # Compile the DTS used by kernel
...
*  Because RK3328 belongs to a 64-bit processor, so select the required file system under the corresponding `Linux Rootfs/arm64` folder. This article uses `rk3399_ubuntu18.04.img.7z` as an example.
...
#Modify roc-rk3328-pc-ubuntu.mk :
...
`parameter.txt` contains firmware partition information is very important. You can find some `parameter.txt` files in `device/rockchip/RK3328` directory. The following is introduced with `parameter.txt` as an example:
...
[18.04 FS]: https://en.t-firefly.com/doc/download/77.html#other_117
[Download the Ubuntu root filesystem image]: https://en.t-firefly.com/doc/download/77.html#other_117