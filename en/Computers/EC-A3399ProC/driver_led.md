...
There are 2 LEDs on the EC-A3399ProC development board, as the following table shows:

![](../../../rk3399_img/EC-A3399ProC/led_pin.jpg)

Both LEDs can be controlled by using the LED device subsystem or by directly operating GPIO.

## Controlling LEDs by device

Linux has its own LED subsystem for LED devices. In EC-A3399ProC, 2 LEDs are configured as LED class devices.
...
It is a simple trigger mode to control LEDs, as follows on the default open yellow LED. And EC-A3399ProC's yellow LED will be turned on after boot.
...
Save the configuration and compile the kernel, the `kernel.img` burn EC-A3399ProC board. We can use the serial input command, you can see the blue light non-stop interval flashing.
