...
* **spi-max-frequency :** set the highest frequency used by spi here. EC-A3399ProC supports up to 48000000.
...
Linux provides a SPI user interface with limited functionality. If IRQ or other kernel driver interfaces are not required, consider using `spidev` interface to write user-level programs to control SPI devices. The corresponding path in the EC-A3399ProC development board is `/dev/spidev0.0`.
