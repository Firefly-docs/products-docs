...
The EC-Orin Nano provides 1 HDMI output interface.

## Ethernet

The EC-Orin Nano provides 1 gigabit Ethernet port.

## USB Interface

The EC-Orin Nano provides 2 USB 3.0 ports.

## SIM Card

The SIM card slot of the EC-Orin Nano is used together with a 4G module to provide mobile network connectivity. **The 4G module is optional**, and the SIM card function is available only after the module has been installed inside the chassis. The SIM card insertion direction is shown in the figure below. Please power off the device before inserting or removing the SIM card.
...
The location of the CAN interface of the EC-Orin Nano is [shown in the figure](started.md). Since there is only one CAN, the first device created in the kernel is `can0` by default.
...
Connect the RS485B-A, RS485B-B and GND pins of the EC-Orin Nano to the A, B and GND pins of the host serial port adapter (USB to 485 serial port module) respectively.
...
The EC-Orin Nano has two audio outputs and one audio input.
...
The EC-Orin Nano has one internal RTC powered by an external motherboard battery, represented as `rtc0` in the kernel.
