...
**A:** The Station P1 Pro has three boot modes:
...
**Debug serial port as an aid**

If a debug serial port is connected to the device, you can also check the U-Boot/kernel boot log in a serial terminal to help determine the current mode. Take MobaXterm on Windows as an example. The serial port configuration is as follows (baud rate 1500000. For the connection method, please refer to [Debug Serial Port](usb_to_ttl.md)):
...
For how to make the device enter the upgradable mode, please refer to [Upgrade the firmware via USB cable](loader_mode.md).

## Q: How to flash partitions?

**A:** The following content is intended for advanced users who need to flash partitions (boot/kernel/rootfs, etc.) separately. If you only need a full firmware upgrade, please refer to [Upgrade the firmware via USB cable](loader_mode.md).
...
![RKDevTool upgrade tool interface](../../../rk3399_img/upgrade_firmware_androidtool.jpg)
</center>

Note: different firmwares may require different tool versions. When using `Androidtool_2.58` to flash `ubuntu(GPT)`, the default configuration works. To flash the `Android8.1` firmware, switch to the `Download Image` page first, right-click the table and select `Import config`, then choose rk3399-Android81.cfg. When using `Androidtool_2.71` to flash `Android10` or `Android9` firmwares, the default configuration works.

### Linux Operating System

#### Flash partition images

For Android7.1 and Android8.1:
...
For Android9.0 and Android10.0:
...
### Upgrade failed / Loader version mismatch

If the upgrade fails, you can try to erase the flash with the `Erase Flash` button first, and then upgrade. If the loader version of the firmware you are flashing is different from the original one on the machine, run `Erase Flash` before the upgrade.
...
For the detailed steps, please refer to [MaskRom mode](03-upgrade_firmware.md).

**Please read carefully and operate carefully!**

### Enter MaskRom mode

The Station P1 Pro computer does not expose a MaskRom button on the enclosure. You need to open the enclosure and find the MaskRom location on the ROC-RK3399-PC Pro motherboard as shown in the figure below. Short-circuit its two contacts with metal tweezers and keep them connected, then power on the device. After a moment, release the tweezers, and the device will enter MaskRom mode.

<center>

<img alt="" src="../../../rk3399_img/ROC-RK3399-PC-Pro/maskrom_test_points.jpg" width="700">
</center>