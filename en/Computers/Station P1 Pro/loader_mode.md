...
The firmware can be obtained by compiling the SDK, or you can download the public firmware (unified firmware) from the [Resource download](https://community.t-firefly.com/en/doc/download/145).There are two types of firmware files:
...
Download [Release_DriverAssistant.zip](https://community.t-firefly.com/en/doc/download/145#windows_341), extract, and then run the DriverInstall.exe inside . In order for all devices to use the updated driver, first select `Driver uninstall`(`驱动卸载`) and then select `Driver install`(`驱动安装`).
...
* Download and run [AndroidTool](https://community.t-firefly.com/en/doc/download/145#other_343)'s RKDevTool.exe
...
Download [Linux_Upgrade_Tool](https://community.t-firefly.com/en/doc/download/145#other_367), And install it into the system as follows for easy invocation:
...
* Type-C data cable connect the device and the host.
    ...
    <img alt="" src="../../../rk3399_img/Station-P1-Pro/upgrade_otg_interface.png" width="900">
    </center>
* Press and hold the RECOVERY button on the device.

    <center>

    <img alt="" src="../../../rk3399_img/Station-P1-Pro/upgrade_recovery_reset.png" width="800">
    </center>
* plug in
