...
The ROC-RK3568-PCSE development board has two kinds of AD interface, respectively: Temperature Sensor and Successive Approximation Register. Among them:
...
### Configure DTS nodes
The SAR-ADC nodes of ROC-RK3568-PCSE defined in `kernel/arch/arm64/boot/dts/rockchip/rk3568.dtsi` file, as showm below:
...
Linux DTS: kernel/arch/arm64/boot/dts/rockchip/.dts

Android DTS: kernel/arch/arm64/boot/dts/rockchip/.dts
```
   adc_demo: adc_demo{
       status = "disabled";
...
The user driver can refer to Firefly adc demo: `kernel/drivers/iio/adc/adc-firefly-demo.c`, which is a driver that detects the status of ROC-RK3568-PCSE's fan. First, define the `of_device_id` structure array in the driver file:
...
Compile the kernel, upgrade the kernel to ROC-RK3568-PCSE development board, and then plug out the fan, the kernel log information will be printed as follows:
