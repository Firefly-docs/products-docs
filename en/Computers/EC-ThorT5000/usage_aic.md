...
AIC (Android in Container) means runing Android inside a container on Linux. Thor Linux can use docker to run Android and support running multiple Androids at the same time.
...
One Thor using AIC running two Genshin Impact with highest graphic setting at the same time can reach 35+ fps:
