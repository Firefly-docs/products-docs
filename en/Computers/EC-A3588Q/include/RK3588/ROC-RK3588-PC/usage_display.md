...
## EC-A3588Q  the configuration of the Display interface  

 EC-A3588Q  supports only two display output interfaces by default (but the 164-pin interface can be extended to VGA, Display Port1 and DSI display interfaces), namely HDMI and Display Port, which can be used for multi-screen display/different display, as shown in the interface diagram below.  
...
The EC-A3588Q hardware has two HDMI display output connectors.
...
EC-A3588Q has a Display Port display output interface, supports DP TX 1.4a protocol, and can support resolutions up to 7680x4320@30Hz.
...
Since EC-A3588Q only supports 3 display outputs by default, HDMI0/HDMI1/Display Port0, there are conventional scenarios configured as follows.
