...
EC-A1684XJD4 FD supports wireless bluetooth, you can display the bluetooth device information through `hciconfig -a` command:
...
(5) At this point, the EC-A1684XJD4 FD Bluetooth device can be found on the smartphone, and the phone can be paired after clicking:
...
The Obex service is needed in Linux. Firstly, connect the EC-A1684XJD4 FD to the Bluetooth device according to the previous steps, then start the Obex daemon, and set the receiving directory to `/home/linaro/`:
...
The following demonstrates the steps for EC-A1684XJD4 FD to receive files:
...
(3) Connect EC-A1684XJD4 FD (master device):
...
(5) Check the `test.txt` file in the `/home/linaro/` directory of EC-A1684XJD4 FD:
