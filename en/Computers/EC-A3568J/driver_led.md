...
There are 2 LEDs on the EC-A3568J development board, as the following table shows:
...
Linux has its own LED subsystem for LED devices. In EC-A3568J, 2 LEDs are configured as LED class devices.
...
It is a simple trigger mode to control LEDs, as follows on the default open yellow LED. And EC-A3568J's yellow LED will be turned on after boot.
...
Save the configuration and compile the kernel, the `kernel.img` burn EC-A3568J board. We can use the serial input command, you can see the blue light non-stop interval flashing.
