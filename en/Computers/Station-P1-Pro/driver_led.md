...
There are 2 LEDs on the Station P1 Pro development board, as the following table shows:

![](../../../rk3399_img/Station-P1-Pro/led_pin.jpg)

Both LEDs can be controlled by using the LED device subsystem or by directly operating GPIO.

## Controlling LEDs by device

Linux has its own LED subsystem for LED devices. In Station P1 Pro, 2 LEDs are configured as LED class devices.
...
It is a simple trigger mode to control LEDs, as follows on the default open yellow LED. And Station P1 Pro's yellow LED will be turned on after boot.
...
Save the configuration and compile the kernel, the `kernel.img` burn Station P1 Pro board. We can use the serial input command, you can see the blue light non-stop interval flashing.
