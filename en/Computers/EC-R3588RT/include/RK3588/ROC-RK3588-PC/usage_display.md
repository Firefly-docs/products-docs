...
## EC-R3588RT  the configuration of the Display interface  

 EC-R3588RT  supports only two display output interfaces by default (but the 164-pin interface can be extended to VGA, Display Port1 and DSI display interfaces), namely HDMI and Display Port, which can be used for multi-screen display/different display, as shown in the interface diagram below.  
...
The EC-R3588RT hardware has two HDMI display output connectors.
...
EC-R3588RT has a Display Port display output interface, supports DP TX 1.4a protocol, and can support resolutions up to 7680x4320@30Hz.
...
Since EC-R3588RT only supports 3 display outputs by default, HDMI0/HDMI1/Display Port0, there are conventional scenarios configured as follows.
