...
EC-A3576C The following figure shows the serial port of the hardware version：

<center>

<img alt="" src="../../../rk3576_img/EC-A3576C/usage_adc_interface.jpg" width="900">
</center>

## Introduction

The AIO-3576C development board has two kinds of AD interface, respectively: Temperature Sensor and Successive Approximation Register. Among them:
...
### Configure DTS nodes
The SAR-ADC nodes of AIO-3576C defined in `kernel/arch/arm64/boot/dts/rockchip/rk3576.dtsi` file, as showm below:

```
saradc: adc@2ae00000 {
		compatible = "rockchip,rk3576-saradc", "rockchip,rk3576-saradc";
...
The user driver can refer to Firefly adc demo: `kernel/drivers/iio/adc/adc-firefly-demo.c`, which is a driver to detect the status of the fan, but the actual ROC-RK3576-PC SARADC channel 4 is not used in the fan, so the driver is only an example.
...
Compile the kernel, upgrade the kernel to AIO-3576C development board, and then plug out the fan, the kernel log information will be printed as follows:
