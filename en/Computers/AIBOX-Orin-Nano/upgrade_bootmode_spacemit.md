...
The AIBOX-OrinNano comes pre-installed with Linux. If the user wants to run other operating systems, the corresponding firmware needs to be flashed onto the motherboard.

The AIBOX-OrinNano offers flexible boot options. Generally, unless there is hardware damage, the AIBOX-OrinNano development board will not become bricked.
...
AIBOX-OrinNano supports firmware upgrades via the following methods:
...
AIBOX-OrinNano Load the system from the following storage:
...
AIBOX-OrinNano has the following boot modes:
