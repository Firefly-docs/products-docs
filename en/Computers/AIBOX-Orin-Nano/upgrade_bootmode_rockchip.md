...
The AIBOX-Orin Nano is installed with the Linux operating system by default. If users want to run other operating systems, they need to use the corresponding firmware to program to the mainboard.

AIBOX-Orin Nano has a flexible startup mode. Generally, the AIBOX-Orin Nano development board will not turn brick unless the hardware is damaged.
...
## Upgrade method
AIBOX-Orin Nano supports firmware update through the following two methods:
...
AIBOX-Orin Nano has three startup modes:
