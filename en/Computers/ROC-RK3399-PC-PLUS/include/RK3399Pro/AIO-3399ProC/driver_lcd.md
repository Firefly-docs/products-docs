...
The ROC-RK3399-PC-PLUS development board supports two LCD screen interfaces by default, one is LVDS and the other is EDP. The position of the interface on the board is as follows:

![](../../../rk3399_img/ROC-RK3399-PC-PLUS/lcd_interface.jpg)

## LVDS screen
### DTS configuration
#### Pin configuration

ROC-RK3399-PC-PLUS SDK has LVDS DSI DTS file: kernel/arch/arm64/boot/dts/rockchip/rk3399pro-firefly-aioc-lvds-HSX101H40C.dts, we can see the following statement from this file:
```
/ {
        model = "ROC-RK3399-PC-PLUS 10.1 inches LVDS HSX101H40C (Android)";
        compatible = "rockchip,rk3399pro-firefly-aioc-lvds", "rockchip,rk3399pro";
...
&dsi {
        ...
        panel@0 {
                ...
                enable-5v-gpios = <&gpio4 RK_PD1 GPIO_ACTIVE_HIGH>; //vcc_5v_en
                enable-tc-gpios = <&gpio0 RK_PB0 GPIO_ACTIVE_HIGH>; //tc358775 power
                reset-gpios = <&gpio4 RK_PD6 GPIO_ACTIVE_LOW>; //tc358775 reset
...
```
bl_en:(GPIO4_D5)GPIO_ACTIVE_HIGH
lcd_en:(GPIO4_D1)GPIO_ACTIVE_HIGH
lcd_pwr_en:(GPIO0_B0)GPIO_ACTIVE_HIGH
lcd_rst:(GPIO4_D6)GPIO_ACTIVE_HIGH
```
...
The ROC-RK3399-PC-PLUS development board has an external backlight interface to control the screen backlight, as shown in the following figure:

![](../../../rk3399_img/ROC-RK3399-PC-PLUS/lcd_back_light.jpg)
...
```
pwms attribute: Configure PWM. In the example, pwm0 is used by default, and 25000ns is the period (40 KHz). LVDS needs to add backlight power control pin. You can see the following statement in kernel/arch/arm64/boot/dts/rockchip/rk3399pro-firefly-aioc-lvds-HSX101H40C.dts:
```
&backlight {
    status = "okay";
...
Unlike the EDP screen, the Timing of the LVDS screen is written in the DTS file. The following statement can be seen in kernel/arch/arm64/boot/dts/rockchip/rk3399pro-firefly-aioc-lvds-HSX101H40C.dts:
...
You can see the initialization instruction list of lvds in kernel/arch/arm64/boot/dts/rockchip/rk3399pro-firefly-aioc-lvds-HSX101H40C.dts:
```
&dsi {
   status = "okay";
        ...
...
### DTS configuration
#### Pin configuration
ROC-RK3399-PC-PLUS SDK has EDP DSI DTS file: kernel/arch/arm64/boot/dts/rockchip/rk3399pro-firefly-aioc-edp.dts, from this file we can see the following statement:
```
{
        panel: panel {
...
The kernel can also write Timing in panel-simple.c, and directly match with short strings. There are the following statements in the drivers/gpu/drm/panel/panel-simple.c file
