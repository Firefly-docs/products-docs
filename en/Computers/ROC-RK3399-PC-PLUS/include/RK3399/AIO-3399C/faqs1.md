...
ROC-RK3399-PC-PLUS The default recording input source is an onboard microphone `Builtin Mic`,You can manually switch to phone recording `Wired Headset`:
...
## ROC-RK3399-PC-PLUS(AI) Type-C interface OTG mode is not normal, connected to the PC can not find ADB device?

Because ROC-RK3399-PC-PLUS(AI) has built-in NPU module, NPU module and Type-C interface use the same set of USB2.0 as RK3399, **official firmware config this set of USB2.0 to HOST mode for NPU module to use by default**, So Type-C interface connected to the PC can not find ADB device.
...
The difference between ROC-RK3399-PC-PLUS(AI) and ROC-RK3399-PC-PLUS defaults is compared in the following table:

|  | ROC-RK3399-PC-PLUS(AI) | ROC-RK3399-PC-PLUS |
|---|---|---|
| Type-C | HOST | OTG |
| USB3.0 | OTG | HOST |

Note: **both ROC-RK3399-PC-PLUS(AI) and ROC-RK3399-PC-PLUS require Type-C interface to upgrade firmware.**

## ROC-RK3399-PC-PLUS(AI) I have purchased the version without NPU. How to configure the Type-C interface to OTG mode and the USB3.0 interface to HOST mode?
