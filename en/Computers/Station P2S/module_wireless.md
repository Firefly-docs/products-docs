...
<img alt="" src="../../../rk356x_img/ROC-RK3568-PC-SE/module_wireless_4G-EC20_MiniPCIe_AntWord.jpg" width="700">
</center>
...
* Connect the VCC, GND, TX and RX of the module to 3.3V, GND, RX and TX of ROC-RK3568-PC-SE **UART7**(<font color = "red">the corresponding node is `/dev/ttyS7`</font>) respectively. Pay attention to avoid<font color="red"> burning the module </font> due to wrong connection of VCC, GND, TX and RX.
...
**Note**: The GPS function will occupy UART7. If UART7 needs to be used for other purposes, the GPS needs to be disabled first.
...
Ensure that the firmware is the latest [public firmware](https://community.t-firefly.com/en/doc/download/180). The firmware turns off the GPS function by default and needs to be enabled manually. The following is the modification method:
...
* Enabled GPS
    * Modify the `BOARD_HAS_GPS` in the SDK directory `device/rockchip/rk356x/rk3568_firefly_roc_pc_se/rk3568_firefly_roc_pc_se.mk` to `true` to enable GPS function. Then, recompile the SDK and upgrad the firmware to take effect.

* Modify serial port configuration (serial port node or baud rate)
    * Modify the `SERIAL_DEVICE` or `SERIAL_BAUD_RATE` in the SDK directory `device/rockchip/rk356x/rk3568_firefly_roc_pc_se/rk3568_firefly_roc_pc_se.mk` to the value of the corresponding module configuration. Then, recompile the SDK and upgrad the firmware to take effect.