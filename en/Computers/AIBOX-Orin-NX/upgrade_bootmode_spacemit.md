...
The AIBOX-OrinNX comes pre-installed with Linux. If the user wants to run other operating systems, the corresponding firmware needs to be flashed onto the motherboard.

The AIBOX-OrinNX offers flexible boot options. Generally, unless there is hardware damage, the AIBOX-OrinNX development board will not become bricked.
...
AIBOX-OrinNX supports firmware upgrades via the following methods:
...
AIBOX-OrinNX Load the system from the following storage:
...
AIBOX-OrinNX has the following boot modes:
