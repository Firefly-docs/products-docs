...
The RKLLM SDK helps users quickly deploy large language models onto AIBOX-Orin NX.
...
```
##### 3.1.2 Deploy And Run The RKLLM Model On AIBOX-Orin NX.
...
./build-linux.sh
```
###### 3.1.2.3 Running Inference On AIBOX-Orin NX
...
AIBOX-Orin NX supports the deployment of a range of mainstream artificial intelligence algorithms, including but not limited to the following categories of algorithms:
...
In addition to the above artificial intelligence algorithms,see [rknn_model_zoo](https://github.com/airockchip/rknn_model_zoo) for more examples.AIBOX-Orin NX also supports large language models. For information on deploying large language models, please refer to the [Large Language Model](usage_llm_rockchip.md) section.

### 1.1 NPU Usage
AIBOX-Orin NX comes with a built-in NPU module,offering processing performance of up to 6 TOPS.To use this NPU,you need to download the [RKNN SDK](https://github.com/airockchip/rknn-toolkit2).The RKNN SDK provides C++/Python programming interfaces, which help users deploy RKNN models exported by RKNN-Toolkit2,accelerating the deployment of AI applications. The overall development steps for RKNN are divided into three main parts: model conversion, model evaluation, board-side deployment and execution.
