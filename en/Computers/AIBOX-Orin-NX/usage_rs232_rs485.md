...
AIBOX-Orin NX has a RS485 interface, the device name is `/dev/ttyS2`, supports full duplex, and the default baud rate is `9600`.
...
In addition to RS485, AIBOX-Orin NX also has an RS232 interface, the device name is `/dev/ttyS1`, supports full duplex, and the default baud rate is `115200`.
