...
AIBOX-Orin NX supports wireless bluetooth, you can display the bluetooth device information through `hciconfig -a` command:
...
(5) At this point, the AIBOX-Orin NX Bluetooth device can be found on the smartphone, and the phone can be paired after clicking:
...
AIBOX-Orin NX does not have this tool installed by default, and users need to compile and install it by themselves. Here are the steps for version 1.3.0, as follows:
...
The Obex service is needed in Linux. Firstly, connect the AIBOX-Orin NX to the Bluetooth device according to the previous steps, then start the Obex daemon, and set the receiving directory to `/home/linaro/`:
...
The following demonstrates the steps for AIBOX-Orin NX to receive files:
...
(3) Connect AIBOX-Orin NX (master device):
...
(5) Check the `test.txt` file in the `/home/linaro/` directory of AIBOX-Orin NX:
