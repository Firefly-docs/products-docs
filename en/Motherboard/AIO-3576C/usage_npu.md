...
The RKNN model can run directly on the RK3576 platform. There are demos under `rknpu2/examples`. Refer to the `README.md` to compile Android or Linux Demo (Need cross-compile environment). You can also just download compiled [demo](https://community.t-firefly.com/en/doc/download/334).

Run yolov5 demo on the AIO-3576C as follows:
```shell
...
#### Run on AIO-3576C NPU connected to the PC

RKNN Toolkit2 runs on the PC and connects to the AIO-3576C through the PC's USB. RKNN Toolkit2 transfers the RKNN model to the NPU device of AIO-3576C to run, and then obtains the inference results, performance information, etc. from the AIO-3576C


* First prepare AIO-3576C environment: update librknnrt.so and run rknn_server
