...
Google Driver [Android14.0 Bundle](https://community.t-firefly.com/en/doc/download/334.html#other_713)。
...
## AIO-3576C product compilation method
...
./FFTools/mkupdate/mkupdate.sh -l rk3576_firefly_aio_3576c-userdebug
```
After packaging, it will be in rockdev/Image-rk3576_firefly_aio_3576c/ Generate unified firmware under the directory: (product name )XXX_XXX_date XXX.img
...
`kernel.img` and `resource.img` for Android14.0 are included in `boot.img`, After compiling the kernel, you need to run the `./mkimage.sh` command in the android root directory to repackage `boot.img`, and then upgrade `boot.img` of the rockdev/Image-rk3576_firefly_aio_3576c/ directory.
...
When compiling, executing `./mkimage.sh` will repackage `boot.img` and `super.img`, and copy other related image files to the directory rockdev/Image-rk3576_firefly_aio_3576c/. The following lists the image files used by general firmware:
