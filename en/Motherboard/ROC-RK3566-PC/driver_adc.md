...
The ROC-RK3566-PC development board has two kinds of AD interface, respectively: Temperature Sensor and Successive Approximation Register. Among them:
...
### Configure DTS nodes
The SAR-ADC nodes of ROC-RK3566-PC defined in `kernel/arch/arm64/boot/dts/rockchip/rk3568.dtsi` file, as showm below:
...
Here is SARADC channel 5, which is not provided for external use in ROC-RK3566-PC and there is no fan interface. Here is only a reference. 

### Matches DTS nodes in the driver file

The user driver can refer to Firefly adc demo: `kernel/drivers/iio/adc/adc-firefly-demo.c`, which is a driver that detects the status of ROC-RK3566-PC's fan. First, define the `of_device_id` structure array in the driver file:
...
Compile the kernel, upgrade the kernel to ROC-RK3566-PC development board, and then plug out the fan, the kernel log information will be printed as follows:
