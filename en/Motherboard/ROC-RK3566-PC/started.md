# Introduction

[Specification]() | [Purchase](https://www.firefly.store/products/roc-rk3566-pc-quad-core-64-bit-mini-computer-1) | [Downloads](https://community.t-firefly.com/en/doc/download/)
 	
ROC-RK3566-PC is a phone sized mini PC board , equipped with Rockchip quad-core processor RK3566 which is configured with dual-core GPU and high-performance NPU, supporting up to 8G RAM. It supports onboard M.2 , WiFi 2.4G/5G and 4G mobile network . Providing various interfaces, the mainboard is suitable for scenarios such as smart NVR, cloud terminal, IoT gateway and industrial control,Detailed information can be found in the [Interface Definition](interface_definition.md) and [Hardware DOC](hardware_doc.md). 


<center>

<img alt="" src="../../../rk356x_img/ROC-RK3566-PC/front.jpg" width="700">
</center>


## Shipping list (for reference only)

* 1 x ROC-RK3566-PC
* 1 x USB Type-A to Type-C data cable

## Accessories

In addition, you may need the following accessories during use:


* USB power supply 5V/2A 
* SD card
* [Serial port module](module_serial.md)
* [Remote control](module_ir.md)  

**Note**: The development board uses USB power supply(5V/2A), please use it with a suitable power supply,Please check the details here [**USB Power**](module_usb_adapter.md)


 <a id="firmware-format"></a>