...
The RKNN model can run directly on the RK3568 platform. There are demos under `rknpu2/examples`. Refer to the `README.md` to compile Android or Linux Demo (Need cross-compile environment). You can also just download compiled [demo](https://community.t-firefly.com/en/doc/download/107).

Run yolov5 demo on the ROC-RK3568-PC as follows:
```shell
...
#### Run on ROC-RK3568-PC NPU connected to the PC

RKNN Toolkit2 runs on the PC and connects to the ROC-RK3568-PC through the PC's USB. RKNN Toolkit2 transfers the RKNN model to the NPU device of ROC-RK3568-PC to run, and then obtains the inference results, performance information, etc. from the ROC-RK3568-PC


* First prepare ROC-RK3568-PC environment: update librknnrt.so and run rknn_server
