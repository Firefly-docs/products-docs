...
* Connect the VCC, GND, TX and RX of the module to 3.3V, GND, RX and TX of AIO-3399Pro-JD4 **UART1**(<font color = "red">the corresponding node is `/dev/ttysWK2`</font>) respectively. Pay attention to avoid<font color="red"> burning the module </font> due to wrong connection of VCC, GND, TX and RX.
...
**Note**: The GPS function will occupy UART1. If UART1 needs to be used for other purposes, the GPS needs to be disabled first.
...
Ensure that the firmware is the latest [public firmware](https://community.t-firefly.com/en/doc/download/61). The firmware turns off the GPS function by default and needs to be enabled manually. The following is the modification method:
