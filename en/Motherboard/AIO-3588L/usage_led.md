...
Linux has its own LED subsystem for LED devices. In Core-3588L, LEDs are configured as LED class devices.You can control them via `/sys/class/leds/`.
...
It is a simple trigger mode to control LEDs, as follows on the default open yellow LED. And Core-3588L's yellow LED will be turned on after boot.
...
--->LED Support
   --->LED Trigger support
      --->LED Timer Trigger
```
Save the configuration and compile the kernel, the `kernel.img` burn Core-3588L board. We can use the serial input command, you can see the blue light non-stop interval flashing.
