...
The RKNN model can run directly on the RK3576 platform. There are demos under `rknpu2/examples`. Refer to the `README.md` to compile Android or Linux Demo (Need cross-compile environment). You can also just download compiled [demo](https://community.t-firefly.com/en/doc/download/page/id/341.html).

Run yolov5 demo on the EXT-iCore-3576Q38 as follows:
```shell
# Platform is a chip platform, such as RK3576, RK3588, RK3566-RK3568, etc
:/ # cd /data/rknn_yolov5_demo_Android/    (use rknn_yolov5_demo_Linux in Linux System)
...
## Non-RKNN Model
For other models like Caffe, TensorFlow, etc, to run on RK3576 platform, conversions are needed. Use RKNN-Toolkit2 to convert other model into RKNN model.
...
#### Run on EXT-iCore-3576Q38 NPU connected to the PC

RKNN Toolkit2 runs on the PC and connects to the EXT-iCore-3576Q38 through the PC's USB. RKNN Toolkit2 transfers the RKNN model to the NPU device of EXT-iCore-3576Q38 to run, and then obtains the inference results, performance information, etc. from the EXT-iCore-3576Q38


* First prepare EXT-iCore-3576Q38 environment: update librknnrt.so and run rknn_server
