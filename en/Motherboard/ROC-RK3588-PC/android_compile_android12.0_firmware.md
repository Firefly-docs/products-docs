...
* The SDK can be obtained by email. Send the order number to <font color=red>sales@t-firefly.com</font> and indicate the required SDK name [firefly_rk3588_android12_git_20240704](https://community.t-firefly.com/en/doc/download/160.html)
...
Google Driver [Android12.0 Bundle](https://community.t-firefly.com/en/doc/download/160.html)。



## ROC-RK3588-PC product compilation method
...
msk ARCH=arm64   BOOT_IMG=../rockdev/Image-roc_rk3588_pc/boot.img roc-rk3588-pc.img -j8
```
...
`kernel.img` and `resource.img` for Android12.0 are included in `boot.img`, After compiling the kernel, you need to run the `./mkimage.sh` command in the android root directory to repackage `boot.img`, and then upgrade `boot.img` of the rockdev/Image-roc_rk3588_pc/ directory.
...
When compiling, executing `./mkimage.sh` will repackage `boot.img` and `super.img`, and copy other related image files to the directory rockdev/Image-roc_rk3588_pc/. The following lists the image files used by general firmware:
