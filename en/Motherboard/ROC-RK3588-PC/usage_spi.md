...
SPI is a high-speed, full-duplex, synchronous serial communication interface for connecting microcontrollers, sensors, storage devices, etc. The ROC-RK3588-PC development board provides the SPI1 (single chip optional) interface, and the specific position is as follows:
![](../../../rk3588_img/ROC-RK3588-PC/usage_spi_interface.jpg)
...
The hardware connection between ROC-RK3588-PC and XM25QU128C is shown in the following table:
...
* **spi-max-frequency :** set the highest frequency used by spi here. ROC-RK3588-PC supports up to 48000000.
...
Linux provides a SPI user interface with limited functionality. If IRQ or other kernel driver interfaces are not required, consider using `spidev` interface to write user-level programs to control SPI devices. The corresponding path in the ROC-RK3588-PC development board is `/dev/spidev1.0`.
