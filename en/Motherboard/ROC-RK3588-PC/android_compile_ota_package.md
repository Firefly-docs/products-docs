...
```
./FFTools/make.sh -d roc-rk3588-pc -j8 -l roc_rk3588_pc-userdebug -o
```

#### Generate the full package path
```
out/target/product/roc_rk3588_pc/roc_rk3588_pc-ota-eng.xxx.zip 
```
#### Generate the base material package path
```
out/target/product/roc_rk3588_pc/obj/PACKAGING/target_files_intermediates/roc_rk3588_pc-target_files-eng.xxx.zip
```
Copy the base material package and rename it, mainly for subcontracting upgrades.
```
cp out/target/product/roc_rk3588_pc/obj/PACKAGING/target_files_intermediates/roc_rk3588_pc-target_files-eng.xxx.zip ~/ota/v1.zip 
```
...
```
In the `rockdev/Image-roc_rk3588_pc/` directory, find the corresponding firmware and burn it to the machine, then the machine can carry out the full package upgrade and the difference package upgrade,put 'update.zip' into /sdcard/ and restart the full package as prompted.
...
```
 cp out/target/product/roc_rk3588_pc/obj/PACKAGING/target_files_intermediates/roc_rk3588_pc-target_files-eng.xxx.zip  ~/ota/v2.zip
```
...
Refer to [OTA Documentation](https://community.t-firefly.com/en/doc/download/160.html#other_549) to setup the sever.