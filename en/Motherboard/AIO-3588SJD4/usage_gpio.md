...
The AIO-3588SJD4  development board leads out a row of extended GPIO ports for user debugging and development in order to facilitate user development and use. The corresponding pins are as follows:
<center>

<img alt="" src="../../../rk3588_img/Core-3588SJD4/usage_gpio_interface.jpg" width="900">
</center>

This article uses the two general GPIO ports GPIO1_B0 and GPIO1_A4 as examples to write a simple operation GPIO port driver. The path in the SDK is:
...
AIO-3588SJD4  have 5  GPIO bank：GPIO0~GPIO4，Each group was numbered A0~A7, B0~B7, C0~C7, and D0~D7, the following formulas are often used to calculate GPIO Pin:
...
```
For example, if we want to calculate GPIO Pin GPIO1_B0, we could refer to the following step: 

bank = 1;  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;//GPIO<font color=red>1</font>_B0 => 1,  bank ∈ [0,4]

group = 3;  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;//GPIO1_<font color=red>B</font>0 => 1,  group ∈ {(A=0), (B=1), (C=2), (D=3)}

X = 0;    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;//GPIO1_B<font color=red>0</font> => 0, X ∈ [0,7]

number = group * 8 + X = 1 * 8 + 0 = 8

pin = bank*32 + number=  1 * 32 + 8 = 40;

GPIO1_B0 property of dts is described as :<&gpio1 8 GPIO_ACTIVE_HIGH>, by `kernel-5.10/include/dt-bindings/pinctrl/rockchip.h` macro definition ，GPIO1_B0 can also be described as :<&gpio1 RK_PB0 GPIO_ACTIVE_HIGH>。
...
......
```
When GPIO1_D0 pin is not reused by other peripherals, we can export this pin to use
```shell
:/ # ls /sys/class/gpio/
...
/{
    gpio_demo: gpio_demo{
        ...
        pinctrl-0 = <&pin36_40_gpio>;
        firefly-gpio = <&gpio1 RK_PB0 GPIO_ACTIVE_HIGH>;             /*GPIO1_B0*/
        firefly-irq-gpio = <&gpio1 RK_PA4 IRQ_TYPE_EDGE_RISING>;    /*GPIO1_A4*/
    };
};

&pinctrl {
    gpio{
        pin36_40_gpio: pin36_40_gpio{
            rockchip,pins =
