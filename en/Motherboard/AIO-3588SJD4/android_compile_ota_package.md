...
```
./FFTools/make.sh -d aio-3588sjd4 -j8 -l aio_3588sjd4-userdebug -o
```

#### Generate the full package path
```
out/target/product/aio_3588sjd4/aio_3588sjd4-ota-eng.xxx.zip 
```
#### Generate the base material package path
```
out/target/product/aio_3588sjd4/obj/PACKAGING/target_files_intermediates/aio_3588sjd4-target_files-eng.xxx.zip
```
Copy the base material package and rename it, mainly for subcontracting upgrades.
```
cp out/target/product/aio_3588sjd4/obj/PACKAGING/target_files_intermediates/aio_3588sjd4-target_files-eng.xxx.zip ~/ota/v1.zip 
```
...
```
In the `rockdev/Image-aio_3588sjd4/` directory, find the corresponding firmware and burn it to the machine, then the machine can carry out the full package upgrade and the difference package upgrade,put 'update.zip' into /sdcard/ and restart the full package as prompted.
...
```
 cp out/target/product/aio_3588sjd4/obj/PACKAGING/target_files_intermediates/aio_3588sjd4-target_files-eng.xxx.zip  ~/ota/v2.zip
```
...
Refer to [OTA Documentation](https://community.t-firefly.com/en/doc/download/149.html#other_549) to setup the sever.