...
Face-RK3399 V1 exposes one SPI4 interface (multiplexed with GPIO) for external use.

Face-RK3399 V2 exposes one SPI2 interface (multiplexed with GPIO) for external use.

For the actual interface location, see [Interface definition](interface_definition.md).
...
* **spi-max-frequency :** set the highest frequency used by spi here. Face-RK3399 supports up to 48000000.
...
Linux provides a SPI user interface with limited functionality. If IRQ or other kernel driver interfaces are not required, consider using `spidev` interface to write user-level programs to control SPI devices. The corresponding path in the Face-RK3399 development board is `/dev/spidev0.0`.
