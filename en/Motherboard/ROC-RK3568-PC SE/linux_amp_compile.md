...
Download Firefly_Linux_SDK sub-volume compressed package: [rk356x_amp_release_20240607_v0.0.1a](https://community.t-firefly.com/en/doc/download/157.html#other_378)
...
```bash
./build.sh roc-rk3568-pc-se-amp-ubuntu.mk
```
...
roc-rk3568-pc-se-amp-ubuntu.mk                 # ROC-RK3568-PC-SEcorresponds to the board-level configuration script file
```
...
Notice: The kernel device tree is located in the directory kernel/arch/arm64/boot/dts/rockchip/. The device tree rk3568-firefly-roc-pc-se-amp.dts used by it is only compatible with 2GB of memory.
...
```bash
./build.sh roc-rk3568-pc-se-amp-buildroot.mk
```
