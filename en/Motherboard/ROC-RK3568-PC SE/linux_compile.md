...
Download Firefly_Linux_SDK sub-volume compressed package: [Firefly_Linux_SDK Source](https://community.t-firefly.com/en/doc/download/157)
...
[What is Ubuntu Desktop firmware?](ubuntu_desktop_support.md)
...
```bash
./build.sh roc-rk3568-pc-se-ubuntu.mk
```
...
# Kernel dts
export RK_KERNEL_DTS=rk3568-firefly-roc-pc-se # dts file
...
Under `device/rockchip/rk356x/` , in addition to roc-rk3568-pc-se-ubuntu.mk, there are other configuration files that come with different accessories
...
* Download: [Ubuntu rootfs(64-bit)](https://community.t-firefly.com/en/doc/download/157)，put in SDK path
...
```bash
MACHINE=roc-rk3568-pc-se bitbake core-image-minimal
MACHINE=roc-rk3568-pc-se bitbake core-image-minimal-xfce
MACHINE=roc-rk3568-pc-se bitbake core-image-x11
MACHINE=roc-rk3568-pc-se bitbake core-image-sato
```
...
```bash
MACHINE=roc-rk3568-pc-se bitbake core-image-weston
```
...
```bash
MACHINE=roc-rk3568-pc-se bitbake core-image-minimal -c clean
MACHINE=roc-rk3568-pc-se bitbake core-image-sato
```
...
```bash
# kernel
MACHINE=roc-rk3568-pc-se bitbake linux-rockchip
        
# u-boot
MACHINE=roc-rk3568-pc-se bitbake u-boot-rockchip
        
# rkmpp
MACHINE=roc-rk3568-pc-se bitbake rockchip-mpp
        
# rockchip-librga
MACHINE=roc-rk3568-pc-se bitbake rockchip-librga
        
# See more compilation objects
MACHINE=roc-rk3568-pc-se bitbake -s
```
...
```bash
MACHINE=roc-rk3568-pc-se bitbake <target> <paramater>
# e.g
MACHINE=roc-rk3568-pc-se bitbake u-boot-rockchip -c clean
MACHINE=roc-rk3568-pc-se bitbake u-boot-rockchip
```
...
* Download ：[Debian rootfs(64-bit)](https://community.t-firefly.com/en/doc/download/157)，and put it in the SDK
...
```bash
./build.sh roc-rk3568-pc-se-buildroot.mk
```
...
Under `device/rockchip/rk356x/` , in addition to roc-rk3568-pc-se-buildroot.mk, there are other configuration files that come with different accessories
