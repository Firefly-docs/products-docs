...
The AIO-3588JQ  development board leads out a row of extended GPIO ports for user debugging and development in order to facilitate user development and use. The corresponding pins are as follows:
<center>

<img alt="" src="../../../rk3588_img/iCore-3588JQ/usage_gpio_interface.jpg" width="900">
</center>

<font color=red>Note: GPIO2_C3 is not led to the extension pin in the diagram in baseboard version V1.1 and above </font>

This article uses the two general GPIO ports GPIO2_C3 and GPIO3_A6 as examples to write a simple operation GPIO port driver. The path in the SDK is:


```
kernel-5.10/drivers/gpio/gpio-firefly.c
```
...
AIO-3588JQ  have 5  GPIO bank：GPIO0~GPIO4，Each group was numbered A0~A7, B0~B7, C0~C7, and D0~D7, the following formulas are often used to calculate GPIO Pin:
...
```
For example, if we want to calculate GPIO Pin GPIO2_C3, we could refer to the following step: 
bank = 2;  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;//GPIO<font color=red>2</font>_C3 => 2,  bank ∈ [0,4]

group = 2;  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;//GPIO2_<font color=red>C</font>3 => 2,  group ∈ {(A=0), (B=1), (C=2), (D=3)}

X = 3;    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;//GPIO2_C<font color=red>3</font> => 3, X ∈ [0,7]

number = group * 8 + X = 2 * 8 + 3 = 19

pin = bank*32 + number= 2 * 32 + 19 = 83;

GPIO2_C3 property of dts is described as :<&gpio2 19 GPIO_ACTIVE_HIGH>, by `kernel-5.10/include/dt-bindings/pinctrl/rockchip.h` macro definition ，GPIO2_C3 can also be described as :<&gpio2 RK_PC3 GPIO_ACTIVE_HIGH>。
...
......
```
When GPIO2_C3 pin is not reused by other peripherals, we can export this pin to use
```shell
:/ # ls /sys/class/gpio/
export     gpiochip128  gpiochip300  gpiochip509  gpiochip96
gpiochip0  gpiochip200  gpiochip32   gpiochip64   unexport
:/ # echo 83 > /sys/class/gpio/export
:/ # ls /sys/class/gpio/
export  gpiochip0    gpiochip200  gpiochip32   gpiochip64  unexport
gpio83  gpiochip128  gpiochip300  gpiochip509  gpiochip96
:/ # ls /sys/class/gpio/gpio83
...
/{
    gpio_demo: gpio_demo{
        ...
        pinctrl-0 = <&pin83_102_gpio>;
        firefly-gpio = <&gpio2 RK_PC3 GPIO_ACTIVE_HIGH>;             /*GPIO2_C3*/
        firefly-irq-gpio = <&gpio3 RK_PA6 IRQ_TYPE_EDGE_RISING>;    /*GPIO3_A6*/\
    };
};

&pinctrl {
    gpio{
        pin83_102_gpio: pin83_102_gpio{
            rockchip,pins =
...
If GPIO1_D0 and GPIO1_D1 are found to be reused as I2C1_SDA through IO command, the I2C disabled shall be removed when GPIO1_D0 and GPIO1_D1 are used as GPIO or other functions.  
...
```
**Note:** This **GPIO1_D0** is for example only and is not recommended for practical use
...
In `kernel/arch/arm64/boot/dts/rockchip/rk3588s.dtsi` there are:
...
I2c7 is defined in `kernel-5.10/arch/arm64/boot/dts/rockchip/rk3588s-pinctrl.dtsi`
...
"GPIO UNCLAIMED" ndicates that the pin is used by GPIO that has not been registered. We use the above gpio_demo example to register this pin. The pin information will become "gpio_demo gpio1:56 function gpio group pin56_57_gpio", which is called gpio_demo node is configured with pinctrl. The value of pinctrl is pin56_57_gpio, this pin is also applied as GPIO.
