...
* The SDK can be obtained by email. Send the order number to <font color=red>sales@t-firefly.com</font> and indicate the required SDK name [firefly_rk3588_android14_git_20240822](https://community.t-firefly.com/en/doc/download/161.html)
...
Google Driver [Android14.0 Bundle](https://community.t-firefly.com/en/doc/download/161.html#other_768)。


## iCore-3588JQ product compilation method
...
```
./FFTools/make.sh -d rk3588-firefly-aio-3588q-mipi101-M101014-BE45-A1 -j8 -l rk3588_firefly_aio_3588q_mipi-userdebug
...
./FFTools/mkupdate/mkupdate.sh -l rk3588_firefly_aio_3588q-userdebug
```
After packaging, it will be in rockdev/Image-rk3588_firefly_aio_3588q/ Generate unified firmware under the directory: (product name )XXX_XXX_date XXX.img
...
`kernel.img` and `resource.img` for Android14.0 are included in `boot.img`, After compiling the kernel, you need to run the `./mkimage.sh` command in the android root directory to repackage `boot.img`, and then upgrade `boot.img` of the rockdev/Image-rk3588_firefly_aio_3588q/ directory.
...
When compiling, executing `./mkimage.sh` will repackage `boot.img` and `super.img`, and copy other related image files to the directory rockdev/Image-rk3588_firefly_aio_3588q/. The following lists the image files used by general firmware:
