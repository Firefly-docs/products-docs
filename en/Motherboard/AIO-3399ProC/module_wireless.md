...
* Connect the VCC, GND, TX and RX of the module to 3.3V, GND, RX and TX of AIO-3399ProC **UART0**(<font color = "red">the corresponding node is `/dev/ttysWK2`</font>) respectively. Pay attention to avoid<font color="red"> burning the module </font> due to wrong connection of VCC, GND, TX and RX.
...
Ensure that the firmware is the latest [public firmware](https://community.t-firefly.com/en/doc/download/76). The firmware turns off the GPS function by default and needs to be enabled manually. The following is the modification method:
