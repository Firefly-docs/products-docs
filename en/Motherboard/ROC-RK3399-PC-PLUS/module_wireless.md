...
* Connect the VCC, GND, TX and RX of the module to 3.3V, GND, RX and TX of ROC-RK3399-PC-PLUS **UART0**(<font color = "red">the corresponding node is `/dev/ttyS0`</font>) respectively. Pay attention to avoid<font color="red"> burning the module </font> due to wrong connection of VCC, GND, TX and RX.
...
**Note**: The GPS function will occupy UART0. If UART0 needs to be used for other purposes, the GPS needs to be disabled first.
...
Ensure that the firmware is the latest [public firmware](https://community.t-firefly.com/en/doc/download/131). The firmware turns off the GPS function by default and needs to be enabled manually. The following is the modification method:
...
* Enabled GPS
    * Modify the `BOARD_HAS_GPS` in the SDK directory `device/rockchip/rk3399/rk3399_roc_pc_plus.mk`(The version above Android7.1 is `device/rockchip/rk3399/rk3399_roc_pc_plus/rk3399_roc_pc_plus.mk`) to `true` to enable GPS function. Then, recompile the SDK and upgrad the firmware to take effect.

* Modify serial port configuration (serial port node or baud rate)
    * Modify the `SERIAL_DEVICE` or `SERIAL_BAUD_RATE` in the SDK directory `device/rockchip/rk3399/gps/u-blox.conf`(The version above Android7.1 is `device/rockchip/rk3399/rk3399_roc_pc_plus/gps/u-blox.conf`)  to the value of the corresponding module configuration. Then, recompile the SDK and upgrad the firmware to take effect.
