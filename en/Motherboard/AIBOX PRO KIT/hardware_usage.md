...
There are two ways to login to AIBOX PRO KIT, one is via Console (Debug serial), the other is via HDMI.

### Console Login
Type-C Connects to the Console port. The login account is `root`. By default, the `root password` is not set.<br>
Please refer to the [Hardware Interface Introduction](interface_definition.md).
...
AIBOX PRO KIT has an external RTC powered by a capacitor on the external motherboard, which ensures the RTC continues to run for a short period after power loss. In the kernel, it is represented as `rtc0`.
### Interface Usage
...
AIBOX PRO KIT supports wireless bluetooth, you can display the bluetooth device information through `hciconfig -a` command:
...
(5) At this point, the AIBOX PRO KIT Bluetooth device can be found on the smartphone, and the phone can be paired after clicking:
...
AIBOX PRO KIT does not have this tool installed by default, and users need to compile and install it by themselves. Here are the steps for version 1.3.0, as follows:
...
The Obex service is needed in Linux. Firstly, connect the AIBOX PRO KIT to the Bluetooth device according to the previous steps, then start the Obex daemon, and set the receiving directory to `/home/linaro/`:
...
The following demonstrates the steps for AIBOX PRO KIT to receive files:
...
(3) Connect AIBOX PRO KIT (master device):
...
(5) Check the `test.txt` file in the `/home/linaro/` directory of AIBOX PRO KIT:
...
AIBOX PRO KIT supports wireless WIFI, and the network card name in the system defaults to `wlan0`:
...
<img alt="" src="../../../aibox_img/AIBOX-PRO-KIT/4G.png" width="700">
</center>
...
The CAN [interface location of the AIBOX PRO KIT development board is shown here](interface_definition.md).
