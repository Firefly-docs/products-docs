...
SPI is a high-speed, full-duplex, synchronous serial communication interface for connecting microcontrollers, sensors, storage devices, etc. The AIO-3588SG development board provides the SPI1 (single chip optional) interface, and the specific position is as follows:
<center>

<img alt="" src="../../../rk3588_img/Core-3588SG/usage_spi_interface.jpg" width="900">
</center>
...
The hardware connection between AIO-3588SG and XM25QU128C is shown in the following table:

| XM25QU128C | AIO-3588SG   | 
| ----       | ----        |
| /CS        | SPI1_M2_CS0 |
| D0         | SPI1_M2_RX  |
| GND        | GND         |
...
* **spi-max-frequency :** set the highest frequency used by spi here. AIO-3588SG supports up to 48000000.
...
Linux provides a SPI user interface with limited functionality. If IRQ or other kernel driver interfaces are not required, consider using `spidev` interface to write user-level programs to control SPI devices. The corresponding path in the AIO-3588SG development board is `/dev/spidev1.0`.
