...
A portion of the memory in Core-1688JD4 is allocated to NPU, VPP, and VPU devices, so the memory data obtained using commands like `free -h` may not match the actual memory of the machine.
...
Obtain the memory allocation tool compressed package from the [download center] and transfer it to Core-1688JD4. Execute the following command to unpack the compressed package:
