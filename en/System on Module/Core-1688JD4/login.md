# Login
There are two ways to log in to Core-1688JD4: one is through Type-C, and the other is via network SSH.
...
Core-1688JD4 has two Ethernet ports. By default, at the factory, port 0 (near the USB connector) is set to dynamic IP, while port 1 (near the HDMI port) is set to static IP `192.168.150.1` with a subnet mask of `255.255.255.0`. You can set the PC to `192.168.150.2/24` for the initial login.
...
If the login fails, you can try to see if the PC can `ping` the IP address of Core-1688JD4's port 1. Note that the PC must have an IP in the same subnet, as the PC's IP address may not necessarily be in the same subnet.
