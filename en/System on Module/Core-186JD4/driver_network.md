# NETWORK Usage
Core-186JD4 has three types of networks:
...
Core-186JD4 only has the 4G interface enabled (because the 5G network hardware conflicts with the USB3 port, requiring hardware modification to enable the 5G interface). If you make modifications and the hardware supports 5G, you can only choose one to use between the 4G and 5G interfaces.
Access steps:
* SIM card
...
## Wireless Network
The wireless network interface of Core-186JD4 is a PCIE M.2 interface. Currently, the ko files that have been compiled and built into the system are:
```
