...
The AIO-3399Pro-JD4 development board has two kinds of AD interface, respectively: Temperature Sensor and Successive Approximation Register. Among them:
...
### Configure DTS nodes
The SAR-ADC nodes of AIO-3399Pro-JD4 defined in `kernel/arch/arm64/boot/dts/rockchip/rk3399.dtsi` file, as showm below:
...
Here is SARADC channel 3, which is not provided for external use in AIO-3399Pro-JD4 and there is no fan interface. Here is only a reference. User can use SARADC channel 0 to do some development on your own by referring to this example.
...
Demo is a driver that detects the status of AIO-3399Pro-JD4's fan. First, define the `of_device_id` structure array in the driver file:
...
Compile the kernel, upgrade the kernel to AIO-3399Pro-JD4 development board, and then plug out the fan, the kernel log information will be printed as follows:
