...
Transfer the deployment package to the iCore-1684XQ machine, using the `scp` command is recommended:

1. Run the `ifconfig` command on iCore-1684XQ to view the IP address.
2. Run the `scp` command to transmit the deployment package, for example: `scp llama3.tar linar@ <aibox-ip-addr>:~/`, where <aibox-ip-addr> is the IP address of iCore-1684XQ.
