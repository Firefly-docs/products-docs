...
iCore-1684XQ supports wireless bluetooth, you can display the bluetooth device information through `hciconfig -a` command:
...
(5) At this point, the iCore-1684XQ Bluetooth device can be found on the smartphone, and the phone can be paired after clicking:
...
The Obex service is needed in Linux. Firstly, connect the iCore-1684XQ to the Bluetooth device according to the previous steps, then start the Obex daemon, and set the receiving directory to `/home/linaro/`:
...
The following demonstrates the steps for iCore-1684XQ to receive files:
...
(3) Connect iCore-1684XQ (master device):
...
(5) Check the `test.txt` file in the `/home/linaro/` directory of iCore-1684XQ:
