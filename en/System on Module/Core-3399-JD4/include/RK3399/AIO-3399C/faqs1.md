...
Core-3399-JD4 The default recording input source is an onboard microphone `Builtin Mic`,You can manually switch to phone recording `Wired Headset`:
...
## Core-3399-JD4(AI) Type-C interface OTG mode is not normal, connected to the PC can not find ADB device?

Because Core-3399-JD4(AI) has built-in NPU module, NPU module and Type-C interface use the same set of USB2.0 as RK3399, **official firmware config this set of USB2.0 to HOST mode for NPU module to use by default**, So Type-C interface connected to the PC can not find ADB device.
...
The difference between Core-3399-JD4(AI) and Core-3399-JD4 defaults is compared in the following table:

|  | Core-3399-JD4(AI) | Core-3399-JD4 |
|---|---|---|
| Type-C | HOST | OTG |
| USB3.0 | OTG | HOST |

Note: **both Core-3399-JD4(AI) and Core-3399-JD4 require Type-C interface to upgrade firmware.**

## Core-3399-JD4(AI) I have purchased the version without NPU. How to configure the Type-C interface to OTG mode and the USB3.0 interface to HOST mode?
