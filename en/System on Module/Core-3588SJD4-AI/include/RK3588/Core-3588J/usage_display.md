...
1. When RK3588 does 8K output, in fact, the chip uses the resources of Port0 and Port1 at the same time, but only borrows the Port0 port for output, so when the display controller connected to Port0 is connected to an external 8K display for output and the  Port1 is using. The display controller will work abnormally.
   ...
   For example, when HDMI0 (connected to Port0) is connected to 8K output, the display of HDMI1 (connected to Port1) will be abnormal. Only when the output of Port0 is less than or equal to 4K@60Hz, Port1 can output 4K@60Hz normally.
...
## AIO-3588SJD4-AI Displays the configuration of the interface

 AIO-3588SJD4-AI  There are four display output interfaces, namely HDMI, Display Port, VGA and MIPI DSI, which can achieve multi-screen simultaneous display/exclusive display. The interface diagram is as follows:

* HDMI0/ HDMI1/ Display Port/ VGA
![](../../../rk3588_img/Core-3588SJD4-AI/usage_display_interface.jpg)  

* MIPI DSI0/ MIPI DSI1
![](../../../rk3588_img/Core-3588SJD4-AI/usage_display_dsi_interface.jpg) 
...
* `kernel-5.10/arch/arm64/boot/dts/rockchip/rk3588-firefly-itx-3588j.dts`
* `kernel-5.10/arch/arm64/boot/dts/rockchip/rk3588-firefly-itx-3588j-mipi101-M101014-BE45-A1.dts`



### HDMI

AIO-3588SJD4-AI There are two HDMI display output interfaces on the hardware:

* HDMI0 supports HDMI2.1 protocol, resolution can support up to 7680x4320@60Hz
* HDMI1 supports HDMI2.0 protocol, resolution can support up to 4096x2304@60Hz


#### Software configuration

Since HDMI0 and HDMI1 are similar in software configuration, the following takes the configuration of HDMI0 as an example.
...
It should be noted that since HDMI0 will be connected to Port0 by default, that is, in order to support 8K output. If HDMI1 is connected to the Port1 port at this time, such as:
...
There will be an abnormal display of HDMI1 as mentioned above, so if you want to support HDMI0 (8K) + HDMI1 (4K) scenarios, you need to connect HDMI1 to Port2, such as:

```
diff --git a/kernel-5.10/arch/arm64/boot/dts/rockchip/rk3588-firefly-itx-3588j.dtsi b/kernel-5.10/arch/arm64/boot/dts/rockchip/rk3588-firefly-itx-3588j.dtsi
index afb176b9f8..099cccbf65 100644
--- a/kernel-5.10/arch/arm64/boot/dts/rockchip/rk3588-firefly-itx-3588j.dtsi
+++ b/kernel-5.10/arch/arm64/boot/dts/rockchip/rk3588-firefly-itx-3588j.dtsi
@@ -94,7 +94,7 @@ &hdmi1 {
 &hdmi1_sound {
        status = "okay";
 };
...
AIO-3588SJD4-AI has a Display Port display output interface, supports DP TX 1.4a protocol, and resolution can support up to 7680x4320@30Hz.
...
AIO-3588SJD4-AI has a VGA display output interface, which is implemented by DP to VGA chip. Limited by the VGA protocol, the resolution can support up to 1080p@60Hz.
...
AIO-3588SJD4-AI There are two MIPI DSI display output interfaces, both support DPHY2.0 and 4 Lane data output, and can output up to 4096x2304@60Hz (depending on the connected Portx).
...
Combining AIO-3588SJD4-AI  DSI0 interface and screen timing

* DSI0 interface
![](../../../rk3588_img/Core-3588SJD4-AI/usage_display_mipi_v2_interface.png)
...
## AIO-3588SJD4-AI  Multiscreen scene configuration

Here are some general scenarios about Portx and configuration methods between display controllers:

* Four-screen display (connected to DP screen, HDMI0 + HDMI1 + Display Port + MIPI DSI0)
```
&hdmi0_in_vp0 {
        status = "okay";
};
...
* Four-screen display (connected to VGA screen, HDMI0 + HDMI1 + VGA + MIPI DSI0)
```
&hdmi0_in_vp0 {
        status = "okay";
};
...
**In the above two configurations, HDMI0 cannot access screens with resolution greater than 4096x2304@60Hz.**
