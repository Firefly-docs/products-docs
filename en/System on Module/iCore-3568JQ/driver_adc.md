...
The ITX-3568Q development board has two kinds of AD interface, respectively: Temperature Sensor and Successive Approximation Register. Among them:
...
### Configure DTS nodes
The SAR-ADC nodes of ITX-3568Q defined in `kernel/arch/arm64/boot/dts/rockchip/rk3568.dtsi` file, as showm below:
...
Linux DTS: kernel/arch/arm64/boot/dts/rockchip/rk3568j-firefly-itxq.dts

Android DTS: kernel/arch/arm64/boot/dts/rockchip/rk3568-firefly-itx-3568q.dts
```
   adc_demo: adc_demo{
       status = "disabled";
...
Here is SARADC channel 2

### Matches DTS nodes in the driver file

The user driver can refer to Firefly adc demo: `kernel/drivers/iio/adc/adc-firefly-demo.c`, which is a driver that detects the status of ITX-3568Q's fan. First, define the `of_device_id` structure array in the driver file:
...
Compile the kernel, upgrade the kernel to ITX-3568Q development board, and then plug out the fan, the kernel log information will be printed as follows:
