...
### Configure Sensor
We need to check the MIPI CSI interface in schematic to find out which I2C bus is used for camera sensor. And configure the camera under that I2C node, correctly set the properties like I2C device address, pins, etc. For example, there's a configuration for xc7160 in iCore-3568JQ:
```shell
&i2c4 {
        status = "okay";
        XC7160: XC7160b@1b{
                ...
                pinctrl-0 = <&cif_clk>;
                reset-gpios = <&pca9555_1 PCA_IO0_2 GPIO_ACTIVE_HIGH>;
                pwdn-gpios = <&pca9555_1 PCA_IO0_4 GPIO_ACTIVE_HIGH>;
...
### Configure Sensor
We need to check the MIPI CSI interface in schematic to find out which I2C bus is used for camera sensor. And configure the camera under that I2C node, correctly set the properties like I2C device address, pins, etc. For example, there's a configuration for gc2053/gc2093 in iCore-3568JQ:
```shell
&i2c4 {
            status = "okay";
           gc2053: gc2053@37 { //IR
				...
				power-gpios = <&pca9555_1 PCA_IO0_2 GPIO_ACTIVE_HIGH>;
				pwdn-gpios = <&pca9555_1 PCA_IO0_3 GPIO_ACTIVE_LOW>;
