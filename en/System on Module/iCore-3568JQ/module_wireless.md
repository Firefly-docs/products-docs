...
<img alt="" src="../../../rk356x_img/iCore-3568JQ/module_wireless_4G-EC20_MiniPCIe_AntWord.jpg" width="700">
</center>
...
## [RM500Q-GL 5G Module suite](https://www.firefly.store/products/rm500q-gl-5g-m-2-module)
### Product parameters
* **Model**
...
<img alt="" src="../../../rk356x_img/iCore-3568JQ/module_wireless_5G-RM500Q-GL_NGFF_AntWord.png" width="700">
</center>
...
* SIM Card Connection
    ...
    * refer to [EC20 4G module suite](#ec20-4g-module-suite)
...
* Connect the VCC, GND, TX and RX of the module to 3.3V, GND, RX and TX of ITX-3568Q **UART9**(<font color = "red">the corresponding node is `/dev/ttyS9`</font>) respectively. Pay attention to avoid<font color="red"> burning the module </font> due to wrong connection of VCC, GND, TX and RX.
...
Ensure that the firmware is the latest [public firmware](https://community.t-firefly.com/en/doc/download/163). The firmware turns off the GPS function by default and needs to be enabled manually. The following is the modification method:
...
* Enabled GPS
    * Modify the `BOARD_HAS_GPS` in the SDK directory `device/rockchip/rk356x/rk3568_firefly_itx_3568q/rk3568_firefly_itx_3568q.mk` to `true` to enable GPS function. Then, recompile the SDK and upgrad the firmware to take effect.

* Modify serial port configuration (serial port node or baud rate)
    * Modify the `SERIAL_DEVICE` or `SERIAL_BAUD_RATE` in the SDK directory `device/rockchip/rk356x/rk3568_firefly_itx_3568q/rk3568_firefly_itx_3568q.mk` to the value of the corresponding module configuration. Then, recompile the SDK and upgrad the firmware to take effect.