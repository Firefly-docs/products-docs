...
There are 2 LEDs on the iCore-3568JQ development board, as the following table shows:
...
Linux has its own LED subsystem for LED devices. In iCore-3568JQ, 2 LEDs are configured as LED class devices.
...
Define LED node in file `kernel/arch/arm64/boot/dts/rockchip/rk3568j-firefly-itxq.dtsi`
```
firefly_leds: leds {
	status = "okay";
...
It is a simple trigger mode to control LEDs, as follows on the default open yellow LED. And iCore-3568JQ's yellow LED will be turned on after boot.
...
Save the configuration and compile the kernel, the `kernel.img` burn iCore-3568JQ board. We can use the serial input command, you can see the blue light non-stop interval flashing.
