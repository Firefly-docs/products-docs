...
<img alt="" src="../../../rk3399_img/Core-3399-JD4/module_wireless_mini-pcie.jpg" width="700">
</center>
...
* Connect the VCC, GND, TX and RX of the module to 3.3V, GND, RX and TX of Core-3399-JD4 **UART2**(<font color = "red">the corresponding node is `/dev/ttysWK3`</font>) respectively. Pay attention to avoid<font color="red"> burning the module </font> due to wrong connection of VCC, GND, TX and RX.
...
Ensure that the firmware is the latest [public firmware](https://community.t-firefly.com/en/doc/download/134). The firmware turns off the GPS function by default and needs to be enabled manually. The following is the modification method:
...
* Enabled GPS
    * Modify the `BOARD_HAS_GPS` in the SDK directory `device/rockchip/rk3399/rk3399_firefly_aiojd4.mk`(The version above Android7.1 is `device/rockchip/rk3399/rk3399_firefly_aiojd4/rk3399_firefly_aiojd4.mk`) to `true` to enable GPS function. Then, recompile the SDK and upgrad the firmware to take effect.

* Modify serial port configuration (serial port node or baud rate)
    * Modify the `SERIAL_DEVICE` or `SERIAL_BAUD_RATE` in the SDK directory `device/rockchip/rk3399/gps/u-blox.conf`(The version above Android7.1 is `device/rockchip/rk3399/rk3399_firefly_aiojd4/gps/u-blox.conf`)  to the value of the corresponding module configuration. Then, recompile the SDK and upgrad the firmware to take effect.
