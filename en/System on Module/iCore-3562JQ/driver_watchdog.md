...
CPU has a internal watchdog. External watchdog is optional. We usually use PC9202 as external watchdog.
...
## External watchdog
Here we use PC9202 as example, other model may has difference in timeout.
...
echo 2 > /dev/wdt_crl
```
External watchdog can also be controled by programe, just like the internal one, only needs to change the device name and timeout.
...
3. Now <font color=red>iCore-3562JQ v1.0</font> version, external watchdog cannot be closed in software way after it is enabled, you need to cut the power
