...
* Connect the VCC, GND, TX and RX of the module to 3.3V, GND, RX and TX of AIO-3568J **UART1**(<font color = "red">the corresponding node is `/dev/ttysWK2`</font>) respectively. Pay attention to avoid<font color="red"> burning the module </font> due to wrong connection of VCC, GND, TX and RX.
...
Ensure that the firmware is the latest [public firmware](http://en.t-firefly.com/doc/download/89.html#other_364). The firmware turns off the GPS function by default and needs to be enabled manually. The following is the modification method:
...
* Enabled GPS
    * Modify the `BOARD_HAS_GPS` in the SDK directory `device/rockchip/rk356x/rk3568_firefly_aioj/rk3568_firefly_aioj.mk` to `true` to enable GPS function. Then, recompile the SDK and upgrad the firmware to take effect.

* Modify serial port configuration (serial port node or baud rate)
    * Modify the `SERIAL_DEVICE` or `SERIAL_BAUD_RATE` in the SDK directory `device/rockchip/rk356x/rk3568_firefly_aioj/rk3568_firefly_aioj.mk` to the value of the corresponding module configuration. Then, recompile the SDK and upgrad the firmware to take effect.