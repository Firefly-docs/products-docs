...
The ROC-RK3506J-CC is installed with the one kind of operating system by default. If users want to run other operating systems, they need to download the corresponding firmware to the mainboard.

ROC-RK3506J-CC has a flexible startup mode. Generally, the ROC-RK3506J-CC development board will not turn brick unless the hardware is damaged.
...
*   [Firmware download link](https://community.t-firefly.com/en/doc/download/296)


## Upgrade method
ROC-RK3506J-CC supports firmware update through the following two methods:
...
Note: The eMMC and SDMMC interfaces are reused, so if the SPI FLASH is not attached, only one of the interfaces can be selected to load the system.

## Boot mode

ROC-RK3506J-CC has three startup modes:
