...
* Identifying the node to capture

1. For cameras using RKISP, such as the SC3336, you need to capture the video node corresponding to rkisp_mainpath. Based on the output information above, rkisp_mainpath corresponds to video11.

   If using the IMX415 camera, use v4l2-ctl to capture camera data frames and save them to /data/out.yuv.
   ```
   ...
   Copy the out.yuv file and view it using Ubuntu.
   ```
