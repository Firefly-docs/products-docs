...
The ITX-3588J  development board leads out a row of extended GPIO ports for user debugging and development in order to facilitate user development and use. The corresponding pins are as follows:
![](../../../rk3588_img/Core-3588J/usage_gpio_interface.jpg)

This article uses the two general GPIO ports GPIO1_D0 and GPIO1_D1 as examples to write a simple operation GPIO port driver. The path in the SDK is:
...
ITX-3588J  have 5  GPIO bank：GPIO0~GPIO4，Each group was numbered A0~A7, B0~B7, C0~C7, and D0~D7, the following formulas are often used to calculate GPIO Pin:
...
For example, if we want to calculate GPIO Pin GPIO1_D0, we could refer to the following step: 
...
GPIO1_D0 property of dts is described as :<&gpio1 24 GPIO_ACTIVE_HIGH>, by `kernel-5.10/include/dt-bindings/pinctrl/rockchip.h` macro definition ，GPIO1_D0 can also be described as :<&gpio1 RK_PD0 GPIO_ACTIVE_HIGH>。
...
In addition, GPIO1 and GPIO2 are extended from the extended GPIO chip(**Not rk3588 GPIO**),and the extended GPIO correspondence diagram
...
The following example is to set GPIO1 to output and high level.For the GPIO2,simply change the Pin number from 215 to 214
```shell
:/ # ls /sys/class/gpio/
export     gpiochip128  gpiochip32   gpiochip64  unexport
gpiochip0  gpiochip200  gpiochip509  gpiochip96
