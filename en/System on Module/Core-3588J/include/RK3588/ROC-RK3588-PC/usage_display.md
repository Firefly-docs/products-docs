...
## Core-3588J  the configuration of the Display interface  

 Core-3588J  supports only two display output interfaces by default (but the 164-pin interface can be extended to VGA, Display Port1 and DSI display interfaces), namely HDMI and Display Port, which can be used for multi-screen display/different display, as shown in the interface diagram below.  
...
The Core-3588J hardware has two HDMI display output connectors.
...
Core-3588J has a Display Port display output interface, supports DP TX 1.4a protocol, and can support resolutions up to 7680x4320@30Hz.
...
## Core-3588J  Multiscreen scene configuration

Since Core-3588J only supports 3 display outputs by default, HDMI0/HDMI1/Display Port0, there are conventional scenarios configured as follows.
