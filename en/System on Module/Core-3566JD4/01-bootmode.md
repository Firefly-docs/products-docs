...
AIO-3566JD4 has 2 working modes. Under normal circumstances, boot directly into `Normal mode` to start the system normally. If you need to upgrade the board subsystem, you can choose the appropriate `Upgrade mode` to upgrade the firmware according to the situation.

* Note: The small memory version of AIO-3566JD4 is installed with the Ubuntu operating system by default, large memory version is installed with Android by default. If users want to run other operating systems, they need to use the corresponding firmware to program to the mainboard.You can go to the [Firmware download page](https://community.t-firefly.com/en/doc/download/123) to download more firmware.
