...
* The SDK can be obtained by email. Send the order number to <font color=red>sales@t-firefly.com</font> and indicate the required SDK name [firefly_rk3588_android14_git_20240822](https://community.t-firefly.com/en/doc/download/139.html)
...
Google Driver [Android14.0 Bundle](https://community.t-firefly.com/en/doc/download/139.html#other_768)。
...
`kernel.img` and `resource.img` for Android14.0 are included in `boot.img`, After compiling the kernel, you need to run the `./mkimage.sh` command in the android root directory to repackage `boot.img`, and then upgrade `boot.img` of the rockdev/Image-aio_3588l/ directory.
