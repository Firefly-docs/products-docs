...
There are two ways to login to Core-1684JD4, one is via RS232, the other is via network SSH.
...
Core-1684JD4 has two Ethernet interfaces. By default, network port 0 (near the serial port) is set with dynamic IP, while network port 1 (near the HDMI port) is set with static IP `192.168.150.1`, subnet mask `255.255 .255.0`, you can set the PC to `192.168.150.2/24` for initial login.
...
If the login fails, you can try to use the PC to `ping` the IP address of the Core-1684JD4 network port 1. Note that the PC needs to add the IP address of the same network segment, because the IP address of the PC is not necessarily in the same network segment .
