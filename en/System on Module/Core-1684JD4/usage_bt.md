...
Core-1684JD4 supports wireless bluetooth, you can display the bluetooth device information through `hciconfig -a` command:
...
(5) At this point, the Core-1684JD4 Bluetooth device can be found on the smartphone, and the phone can be paired after clicking:
...
The Obex service is needed in Linux. Firstly, connect the Core-1684JD4 to the Bluetooth device according to the previous steps, then start the Obex daemon, and set the receiving directory to `/home/linaro/`:
...
The following demonstrates the steps for Core-1684JD4 to receive files:
...
(3) Connect Core-1684JD4 (master device):
...
(5) Check the `test.txt` file in the `/home/linaro/` directory of Core-1684JD4:
