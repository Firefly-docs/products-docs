...
The ICORE-1126BQ38 is installed with the Linux operating system by default. If users want to run other operating systems, they need to use the corresponding firmware to program to the mainboard.

ICORE-1126BQ38 has a flexible startup mode. Generally, the ICORE-1126BQ38 development board will not turn brick unless the hardware is damaged.
...
*   [Firmware download link](https://community.t-firefly.com/en/doc/download/367)


## Upgrade method
ICORE-1126BQ38 supports firmware update through the following two methods:
...
* [Upgrade the firmware via SD card](upgrade_firmware_sd.md)<br><br>    Use the upgrade card creation tool to make the MicroSD card(Suggest using a card with a capacity of 32GB or less) as an upgrade card, insert the upgrade card into the mainboard, power on, and the machine will automatically perform the upgrade.

## Boot media

ICORE-1126BQ38 loads the system from the following memory:
...
ICORE-1126BQ38 has three startup modes:
...
***Forcibly enter `MaskRom mode`, please refer to the chapter [Upgrade Firmware](upgrade_firmware.md).***