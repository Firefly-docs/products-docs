...
Transfer the deployment package to the Core-1684XJD4 machine, using the `scp` command is recommended:

1. Run the `ifconfig` command on Core-1684XJD4 to view the IP address.
2. Run the `scp` command to transmit the deployment package, for example: `scp llama3.tar linar@ <aibox-ip-addr>:~/`, where <aibox-ip-addr> is the IP address of Core-1684XJD4.
