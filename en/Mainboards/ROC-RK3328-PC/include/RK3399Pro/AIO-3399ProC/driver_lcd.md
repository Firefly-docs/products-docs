...
The ROC-RK3328-PC development board supports two LCD screen interfaces by default, one is LVDS and the other is EDP. The position of the interface on the board is as follows:
...
ROC-RK3328-PC SDK has LVDS DSI DTS file: kernel/arch/arm64/boot/dts/rockchip/rk3399pro-firefly-aioc-lvds-HSX101H40C.dts, we can see the following statement from this file:
```
/ {
        model = "ROC-RK3328-PC 10.1 inches LVDS HSX101H40C (Android)";
...
The ROC-RK3328-PC development board has an external backlight interface to control the screen backlight, as shown in the following figure:
...
### DTS configuration
#### Pin configuration
ROC-RK3328-PC SDK has EDP DSI DTS file: kernel/arch/arm64/boot/dts/rockchip/rk3399pro-firefly-aioc-edp.dts, from this file we can see the following statement:
```
{
        panel: panel {
