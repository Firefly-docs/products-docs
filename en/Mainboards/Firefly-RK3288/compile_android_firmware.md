...
* [[Download Link]](http://en.t-firefly.com/doc/download/4.html#other_35)
...
The `./mkimage.sh` from the previous step will repack the `boot.img` and `system.img` and copy other related image files to the `rockdev/Image-rk3288_box/` directory. The following lists the image files used by the general firmware:
...
The firmware is generated in the `rockdev/Image-rk3288_box/` directory after packing.
