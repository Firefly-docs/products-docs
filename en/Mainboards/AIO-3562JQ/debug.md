...
AIO-3562JQ use the following serial parameters:
...
Linux normally enabled adb by default, connect device and PC with usb cable, run "adb devices" on PC and you will see the device.

If PC still shows no device, probably because the OTG port is in HOST mode. Run these commands on device to switch it to OTG mode:
```bash
systemctl stop set-otg-mode.service
systemctl disable set-otg-mode.service
systemctl restart adbd.service
```

If you want to switch back to HOST mode, run:
```bash
systemctl enable set-otg-mode.service
reboot
```