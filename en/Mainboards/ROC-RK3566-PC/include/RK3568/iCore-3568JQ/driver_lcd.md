...
ROC-RK3566-PC SDK has MIPI DSI DTS file:`rk3568-firefly-itx-3568q-mipi101_M101014_BE45_A1.dts`, this file is a MIPI_DSI1+HDMI screen configration, MIPI_DSI0 is disabled by default, we would take the dsi1 configuration as an example to introduce the related LCD driver.
...
On the hardware signal, the LCD_EN is active at high level and LCD_RST is active at low levle. Please refer to the section ["GPIO Usage"](driver_gpio.html) for specific pin configuration. It should be noted that ROC-RK3566-PC dsi1's LCD_EN(PCA_IO1_2), this GPIO was from PCA9555 expansion chip, which can only be controlled when the drive is running.
