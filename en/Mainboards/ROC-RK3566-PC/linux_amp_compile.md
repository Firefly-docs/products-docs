...
Download Firefly_Linux_SDK sub-volume compressed package: [rk356x_amp_release_20240607_v0.0.1a](http://en.t-firefly.com/doc/download/93.html#other_378)
...
```bash
./build.sh roc-rk3566-pc-amp-ubuntu.mk
```
...
roc-rk3566-pc-amp-ubuntu.mk                 # ROC-RK3566-PCcorresponds to the board-level configuration script file
```
...
Notice: The kernel device tree is located in the directory kernel/arch/arm64/boot/dts/rockchip/. The device tree rk3566-firefly-roc-pc-amp.dts used by it is only compatible with 2GB of memory.
...
```bash
./build.sh roc-rk3566-pc-amp-buildroot.mk
```
