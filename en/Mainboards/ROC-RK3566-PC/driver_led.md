...
There are 2 LEDs on the ROC-RK3566-PC development board, as the following table shows:
...
Linux has its own LED subsystem for LED devices. In ROC-RK3566-PC, 2 LEDs are configured as LED class devices.
...
Define LED node in file `kernel/arch/arm64/boot/dts/rockchip/rk3566-roc-pc.dtsi`
```
firefly_leds: leds {
...
It is a simple trigger mode to control LEDs, as follows on the default open yellow LED. And ROC-RK3566-PC's yellow LED will be turned on after boot.
...
Save the configuration and compile the kernel, the `kernel.img` burn ROC-RK3566-PC board. We can use the serial input command, you can see the blue light non-stop interval flashing.
