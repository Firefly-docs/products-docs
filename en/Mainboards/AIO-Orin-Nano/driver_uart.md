...
AIO-OrinNano Serial Port [location as shown in the figure](interface_definition.html#zheng-ji-jie-kou-ding-yi)  

## Introduction
AIO-OrinNano supports 3 UART devices:
...
The device file for RS485 is `/dev/ttyS4`. Run the following commands on the device (**Note: Before running the commands, the RS485 send function must be enabled, as AIO-OrinNano has a GPIO switch to toggle RS485 between send and receive states.**):
