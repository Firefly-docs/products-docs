...
A portion of the memory in AIO-Orin Nano is allocated to NPU, VPP, and VPU devices, so the memory data obtained using commands like `free -h` may not match the actual memory of the machine.
...
Obtain the memory allocation tool compressed package from the [download center] and transfer it to AIO-Orin Nano. Execute the following command to unpack the compressed package:
...
[Download Center]: https://www.t-firefly.com/doc/download/322.html