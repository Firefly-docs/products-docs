...
## Firefly-RK3399(AI) Type-C interface OTG mode is not normal, connected to the PC can not find ADB device?

Because Firefly-RK3399(AI) has built-in NPU module, NPU module and Type-C interface use the same set of USB2.0 as RK3399, **official firmware config this set of USB2.0 to HOST mode for NPU module to use by default**, So Type-C interface connected to the PC can not find ADB device.
...
The difference between Firefly-RK3399(AI) and Firefly-RK3399 defaults is compared in the following table:

|  | Firefly-RK3399(AI) | Firefly-RK3399 |
|---|---|---|
| Type-C | HOST | OTG |
| USB3.0 | OTG | HOST |

Note: **both Firefly-RK3399(AI) and Firefly-RK3399 require Type-C interface to upgrade firmware.**

## Firefly-RK3399(AI) I have purchased the version without NPU. How to configure the Type-C interface to OTG mode and the USB3.0 interface to HOST mode?
