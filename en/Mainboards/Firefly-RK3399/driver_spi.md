...
## Introduction
SPI is a high-speed, full-duplex, synchronous serial communication interface for connecting microcontroller, sensors, storage devices, etc. The Firefly-RK3399 development board provides the SPI1 (single chip optional) interface, and the specific position is as follows:
...
The hardware connection between Firefly-RK3399 and W25Q128FV is shown in the following table:
...
* **spi-max-frequency :** set the highest frequency used by spi here. Firefly-RK3399 supports up to 48000000.
...
Linux provides a SPI user interface with limited functionality. If IRQ or other kernel driver interfaces are not required, consider using `spidev` interface to write user-level programs to control SPI devices. The corresponding path in the Firefly-RK3399 development board is `/dev/spidev0.0`.
