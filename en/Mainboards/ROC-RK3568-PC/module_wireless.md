...
* Connect the VCC, GND, TX and RX of the module to 3.3V, GND, RX and TX of ROC-RK3568-PC **UART9**(<font color = "red">the corresponding node is `/dev/ttyS9`</font>) respectively. Pay attention to avoid<font color="red"> burning the module </font> due to wrong connection of VCC, GND, TX and RX.

* For some definitions and descriptions of UART, you can refer to the wiki tutorial [UART](driver_uart.html).


**Note**：ROC-RK3568-PC UART9 is multiplexed with I2S3/SPI3. Actually，the `TX` and `RX` of GPS module need connect with `I2S3 SDI`(UART9 RX)、`I2S3 SDO`(UART9 TX).
...
Ensure that the firmware is the latest [public firmware](http://en.t-firefly.com/doc/download/94.html#other_407). The firmware turns off the GPS function by default and needs to be enabled manually. The following is the modification method:
...
* Enabled GPS
    * Modify the `BOARD_HAS_GPS` in the SDK directory `device/rockchip/rk356x/rk3568_firefly_roc_pc/rk3568_firefly_roc_pc.mk` to `true` to enable GPS function. Then, recompile the SDK and upgrad the firmware to take effect.

* Modify serial port configuration (serial port node or baud rate)
    * Modify the `SERIAL_DEVICE` or `SERIAL_BAUD_RATE` in the SDK directory `device/rockchip/rk356x/rk3568_firefly_roc_pc/rk3568_firefly_roc_pc.mk` to the value of the corresponding module configuration. Then, recompile the SDK and upgrad the firmware to take effect.