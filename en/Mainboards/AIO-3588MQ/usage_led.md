...
| ----    | ----      | ----       |
| Green   | GPIO3_B2  | 106        | 
| Yellow  | PCA_IO0_3 | 203        |

PCA_IO0_3 comes from a GPIO extension chip.

LEDs can be controlled by using the LED device subsystem or by directly operating GPIO.

## Controlling LEDs by device

Linux has its own LED subsystem for LED devices. In iCore-3588MQ, LEDs are configured as LED class devices.You can control them via `/sys/class/leds/`.


* Green:  Turn on after the system powers on.
...
Define LED node in file `kernel-5.10/arch/arm64/boot/dts/rockchip/rk3588-firefly-port.dtsi` 
```
firefly_leds: leds {
...
```
Board :`kernel-5.10/arch/arm64/boot/dts/rockchip/rk3588-firefly-aio-3588q.dtsi`：
```
/* led */
&power_led{
...
It is a simple trigger mode to control LEDs, as follows on the default open yellow LED. And iCore-3588MQ's yellow LED will be turned on after boot.
...
--->LED Support
   --->LED Trigger support
      --->LED Timer Trigger
```
Save the configuration and compile the kernel, the `kernel.img` burn iCore-3588MQ board. We can use the serial input command, you can see the yellow light non-stop interval flashing.
