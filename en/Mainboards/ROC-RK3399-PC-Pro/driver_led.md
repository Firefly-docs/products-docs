...
There are 2 LEDs on the ROC-RK3399-PC Pro development board, as the following table shows:
...
Linux has its own LED subsystem for LED devices. In ROC-RK3399-PC Pro, 2 LEDs are configured as LED class devices.
...
It is a simple trigger mode to control LEDs, as follows on the default open yellow LED. And ROC-RK3399-PC Pro's yellow LED will be turned on after boot.
...
Save the configuration and compile the kernel, the `kernel.img` burn ROC-RK3399-PC Pro board. We can use the serial input command, you can see the blue light non-stop interval flashing.
