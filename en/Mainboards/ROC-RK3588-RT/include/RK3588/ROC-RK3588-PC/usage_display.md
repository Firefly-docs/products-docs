...
## ROC-RK3588-RT  the configuration of the Display interface  

 ROC-RK3588-RT  supports only two display output interfaces by default (but the 164-pin interface can be extended to VGA, Display Port1 and DSI display interfaces), namely HDMI and Display Port, which can be used for multi-screen display/different display, as shown in the interface diagram below.  
...
The ROC-RK3588-RT hardware has two HDMI display output connectors.
...
ROC-RK3588-RT has a Display Port display output interface, supports DP TX 1.4a protocol, and can support resolutions up to 7680x4320@30Hz.
...
Since ROC-RK3588-RT only supports 3 display outputs by default, HDMI0/HDMI1/Display Port0, there are conventional scenarios configured as follows.
