...
The AIO-3588JQ  development board has two kinds of AD interface, respectively: Temperature Sensor and Successive Approximation Register. Among them:
...
### Configure DTS nodes
The SAR-ADC nodes of AIO-3588JQ  defined in `kernel/arch/arm64/boot/dts/rockchip/rk3588s.dtsi` file, as showm below:
...
The user driver can refer to Firefly adc demo: `kernel/drivers/iio/adc/adc-firefly-demo.c`, which is a driver that detects the status of AIO-3588JQ 's fan. First, define the `of_device_id` structure array in the driver file:
...
Compile the kernel, upgrade the kernel to AIO-3588JQ  development board, and then plug out the fan, the kernel log information will be printed as follows:
