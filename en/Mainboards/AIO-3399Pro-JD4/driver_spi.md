...
The hardware connection between AIO-3399Pro-JD4 and W25Q128FV is shown in the following table:
...
* **spi-max-frequency :** set the highest frequency used by spi here. AIO-3399Pro-JD4 supports up to 48000000.
...
Linux provides a SPI user interface with limited functionality. If IRQ or other kernel driver interfaces are not required, consider using `spidev` interface to write user-level programs to control SPI devices. The corresponding path in the AIO-3399Pro-JD4 development board is `/dev/spidev0.0`.
