...
ROC-RK3588-PC The following figure shows the serial port of the hardware version：

![](../../../rk3588_img/ROC-RK3588-PC/usage_adc_interface.jpg)

## Introduction

The ROC-RK3588-PC development board has two kinds of AD interface, respectively: Temperature Sensor and Successive Approximation Register. Among them:
...
### Configure DTS nodes
The SAR-ADC nodes of ROC-RK3588-PC defined in `kernel/arch/arm64/boot/dts/rockchip/rk3588s.dtsi` file, as showm below:
...
* The application here is for saradc channel 6. `For reference only, the actual hardware interfaces prevail.`

### Matches DTS nodes in the driver file

The user driver can refer to Firefly adc demo: `kernel/drivers/iio/adc/adc-firefly-demo.c`, which is a driver that detects the status of ROC-RK3588-PC's fan. First, define the `of_device_id` structure array in the driver file:
...
Compile the kernel, upgrade the kernel to ROC-RK3588-PC development board, and then plug out the fan, the kernel log information will be printed as follows:
