...
There are 2 LEDs on the Face-RK3399 development board, as the following table shows:
...
Linux has its own LED subsystem for LED devices. In Face-RK3399, 2 LEDs are configured as LED class devices.
...
For the Face-X1 face recognition all-in-one device, the three control LEDs on the fill-light board correspond to:
```
...
The LED node is defined in `kernel/arch/arm64/boot/dts/rockchip/rk3399-firefly-face.dtsi`:
```
face_leds {
    compatible = "gpio-leds";
...
For the Face-X2 face recognition all-in-one device, the three control LEDs on the fill-light board correspond to:
```
/sys/class/leds/face:green:user/brightness //green LED
/sys/class/leds/face:red:user/brightness //red LED
/sys/pwm/face_white_led //white LED
```

The white LED brightness is adjustable:
```
echo x > /sys/pwm/face_white_led
```
The value range of `x` is 0 to 8.

Note: The value of `compatible` must match the one in `drivers/leds/leds-gpio.c`.

### Simple trigger LED

It is a simple trigger mode to control LEDs, as follows on the default open yellow LED. And Face-RK3399's yellow LED will be turned on after boot.
...
Save the configuration and compile the kernel, the `kernel.img` burn Face-RK3399 board. We can use the serial input command, you can see the blue light non-stop interval flashing.
