...
AIO-3399ProCThe development board supports two LCD screen interfaces by default, one is LVDS and the other is EDP. The positions of the interfaces on the board are as follows:
...
AIO-3399ProC SDK has LVDS DSI DTS file: kernel/arch/arm64/boot/dts/rockchip/rk3399pro-firefly-aiojd4-lvds-HSX101H40C.dts, we can see the following statement from this file:
```
/ {
...
AIO-3399ProC The development board is equipped with a backlight interface to control the screen backlight, as shown in the figure below:
...
### DTS configuration
#### Pin configuration
The SDK of AIO-3399ProC has the DTS file of EDP DSI: kernel/arch/arm64/boot/dts/rockchip/rk3399pro-firefly-aiojd4-edp.dts, from which we can see the following statement:
```
  edp_panel: edp-panel {
		/* config 2 */
