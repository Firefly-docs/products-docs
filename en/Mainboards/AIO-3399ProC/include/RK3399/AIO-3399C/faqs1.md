...
AIO-3399ProC The default recording input source is an onboard microphone `Builtin Mic`,You can manually switch to phone recording `Wired Headset`:
...
## AIO-3399ProC(AI) Type-C interface OTG mode is not normal, connected to the PC can not find ADB device?

Because AIO-3399ProC(AI) has built-in NPU module, NPU module and Type-C interface use the same set of USB2.0 as RK3399, **official firmware config this set of USB2.0 to HOST mode for NPU module to use by default**, So Type-C interface connected to the PC can not find ADB device.
...
The difference between AIO-3399ProC(AI) and AIO-3399ProC defaults is compared in the following table:

|  | AIO-3399ProC(AI) | AIO-3399ProC |
|---|---|---|
| Type-C | HOST | OTG |
| USB3.0 | OTG | HOST |

Note: **both AIO-3399ProC(AI) and AIO-3399ProC require Type-C interface to upgrade firmware.**

## AIO-3399ProC(AI) I have purchased the version without NPU. How to configure the Type-C interface to OTG mode and the USB3.0 interface to HOST mode?
