# NETWORK Usage
AIO-Orin NX has three types of networks:
* Cellular Network (4G/5G)
* Wireless Network (WIFI/BT)
* Gigabit Ethernet Port

AIO-Orin NX has enabled these three networks by default, depending on user usage. [The location of each interface is shown in the diagram](interface_definition.html#zheng-ji-jie-kou-ding-yi).

## Cellular Network

AIO-Orin NX only has the 4G interface enabled (because the 5G network hardware conflicts with the USB3 port, requiring hardware modification to enable the 5G interface). If you make modifications and the hardware supports 5G, you can only choose one to use between the 4G and 5G interfaces.
Access steps:
* SIM card
...
## Wireless Network
The wireless network interface of AIO-Orin NX is a PCIE M.2 interface. Currently, the ko files that have been compiled and built into the system are:
```
