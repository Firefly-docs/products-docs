...
* **spi-max-frequency :** set the highest frequency used by spi here. ROC-RK3399-PC-PLUS supports up to 48000000.
...
Linux provides a SPI user interface with limited functionality. If IRQ or other kernel driver interfaces are not required, consider using `spidev` interface to write user-level programs to control SPI devices. The corresponding path in the ROC-RK3399-PC-PLUS development board is `/dev/spidev0.0`.
