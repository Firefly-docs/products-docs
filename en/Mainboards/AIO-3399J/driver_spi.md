...
## Introduction
SPI is a high-speed, full-duplex, synchronous serial communication interface for connecting microcontroller, AIO-3399J board provides the SPI2 (single chip optional) interface, and the specific position is as follows:
...
The hardware connection between AIO-3399J and W25Q128FV is shown in the following table:
...
* **spi-max-frequency :** set the highest frequency used by spi here. AIO-3399J supports up to 48000000.
...
Linux provides a SPI user interface with limited functionality. If IRQ or other kernel driver interfaces are not required, consider using `spidev` interface to write user-level programs to control SPI devices. The corresponding path in the AIO-3399J development board is `/dev/spidev0.0`.
