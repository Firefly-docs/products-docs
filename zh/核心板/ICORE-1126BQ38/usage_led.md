...
标准的 Linux 专门为 LED 设备定义了 LED 子系统。 在 ICORE-1126BQ38开发板中的LED 均以设备的形式被定义。用户可以通过 `/sys/class/leds/` 目录控制LED。
...
```
echo 1 > /sys/class/leds/\:power/brightness #黄灯亮
echo 0 > /sys/class/leds/\:power/brightness #黄灯灭
```
...
板级的配置在`sdk/kernel-6.1/arch/arm64/boot/dts/rockchip/rv1126b-firefly-cam-1126bq38.dtsi`：
...
--->LED Support
   --->LED Trigger support
      --->LED Timer Trigger
```
保存配置并编译内核，把 `boot.img` 烧到 ICORE-1126BQ38板子上 我们可以使用串口输入命令，就可以看到蓝灯不停的间隔闪烁。
