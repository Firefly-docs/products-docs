...
RK3576 内置 NPU 模块, 处理性能最高可达6TOPS。使用该NPU需要下载[RKNN SDK](https://community.t-firefly.com/doc/download/302.html)，RKNN SDK 为带有 NPU 的 RK 系列芯片提供编程接口，能够帮助用户部署使用 RKNN-Toolkit2 导出的 RKNN 模型，加速 AI 应用的落地，具体支持平台列表可通过 SDK 中 README 文档或者访问[airockchip](https://github.com/airockchip/rknn-toolkit2/) 查看，开发建议以[airockchip](https://github.com/airockchip/rknn-toolkit2/)官方更新版本作为参考。

## RKNN 模型
RKNN 是 Rockchip NPU 平台使用的模型类型，以`.rknn`后缀结尾的模型文件。用户可以通过RKNN SDK提供的工具将自主研发的算法模型转换成 RKNN 模型

RKNN 模型可以直接运行在 RK3576 平台上，在`rknpu2/examples`中有例子，根据`README.md`编译生成 Android 或 Linux Demo（需要交叉编译环境）。也可以直接下载编译好的 [Demo](https://community.t-firefly.com/doc/download/302.html)。

在 AIO-3576JQ 上运行yolov5 demo如下:
```shell
...
#### 运行在与 PC 相连的 AIO-3576JQ NPU 平台上

RKNN-Toolkit2 通过 PC 的 USB 连接到 OTG 设备 AIO-3576JQ。RKNN-Toolkit2
将 RKNN 模型传到 AIO-3576JQ 的 NPU 上运行，再从 AIO-3576JQ 上获取推理结果、性能信息等：


* 首先部署 AIO-3576JQ 环境：更新librknnrt.so及运行rknn_server 
*Android*
```shell
...
## 详细开发文档
AIO-3576JQ NPU 及 Toolkit 详细使用方法请参考RKNN SDK下《Rockchip_RKNPU_User_Guide_RKNN_API_\*.pdf》、《Rockchip_User_Guide_RKNN_Toolkit2_\*.pdf》文档。
