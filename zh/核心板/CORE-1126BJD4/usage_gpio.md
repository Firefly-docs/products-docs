...
当 GPIO1_B7 脚没有被其它外设复用时，我们可以通过 export 导出该引脚去使用
...
GPIO 口除了通用输入输出、中断功能外，还可能有其它复用功能，以 GPIO7_A4 为例，就有如下几个功能：

|  func0  |  func1  |  func2  |
| --- | --- | --- |
| PDM_CLK0_M0 | I2C4_SDA_M3 | UART2_CTSN_M1 |

查看 `/sys/kernel/debug/pinctrl/pinctrl-rockchip-pinctrl/pinmux-pins`，查看各个引脚的作用，如果发现 GPIO7_A4 被复用为 I2C4_SDA_M3 ，则需要在dts中关闭它。

## 调试方法

### IO指令

GPIO 调试有一个很好用的工具，那就是 IO 指令，CORE-1126BJD4 的 Linux 系统默认已经内置了 IO 指令，使用 IO 指令可以实时读取或写入每个 IO 口的状态，这里简单介绍 IO 指令的使用。首先查看 IO 指令的帮助：
