...
* 将模组的VCC、GND、TX、RX分别接到AIO-3568J **UART1**(<font color = "red">对应节点为`/dev/ttysWK2`</font>)的3.3V、GND、RX、TX，注意避免接错VCC、GND、TX、RX<font color = "red">导致烧坏模组</font>
...
确保固件为最新[公版固件](https://www.t-firefly.com/doc/download/103.html#other_434)，固件默认关闭GPS功能，需要手动使能，以下是修改方法：
...
* 使能GPS
    * 修改SDK目录下`device/rockchip/rk356x/rk3568_firefly_aioj/rk3568_firefly_aioj.mk`里面`BOARD_HAS_GPS`参数为`true`，实现固件使能GPS功能，修改后重新编译SDK并烧录固件生效。

* 修改串口配置(串口节点或者波特率)
    * 将SDK目录下`device/rockchip/rk356x/rk3568_firefly_aioj/gps/u-blox.conf`里面`SERIAL_DEVICE`或者`SERIAL_BAUD_RATE`修改成对应模组配置的值，修改后重新编译SDK并烧录固件生效