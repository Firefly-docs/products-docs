...
AIO-3568J开发板默认外置支持了两个MIPI屏接口和一个EDP屏接口，该接口对应板子上的位置如下图：
![](../../img/Core-3568J/lcd_interface.jpg)

## Config配置

AIO-3568J的SDK中的`kernel/arch/arm64/configs/firefly_defconfig`文件已经把LCD相关的配置设置好了，如果自己做了修改，请注意把以下配置加上：
```
...
AIO-3568J的SDK有MIPI DSI的DTS文件：`kernel/arch/arm64/boot/dts/rockchip/rk3566-firefly-aiojd4-mipi101_JDM101014_BC45_A1.dts`，接下来以此DTS中的dsi0配置作为范例来进行相关的LCD驱动介绍。
...
## EDP DTS配置
### 引脚配置
AIO-3568J的SDK有EDP的DTS文件：`kernel/arch/arm64/boot/dts/rockchip/rk3566-firefly-aiojd4-edp_M156X40.dts`，从该文件中我们可以看到以下语句：
```
