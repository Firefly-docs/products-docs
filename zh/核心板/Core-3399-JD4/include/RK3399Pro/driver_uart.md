...
Core-3399-JD4 支持 SPI 桥接/扩展 4 个增强功能串口（UART）的功能，分别为 UART1，UART2，RS232，RS485。每个 UART 都拥有 256 字节的 FIFO 缓冲区，用于数据接收和发送。其中：
...
Core-3399-JD4 开发板的串口接口图如下：

![](../../../rk3399_img/Core-3399-JD4/uart_interface.jpg)
...
可以看到，在 `kernel/arch/arm64/boot/dts/rockchip/rk3399-firefly-aiojd4.dtsi` 文件中使能该节点即可使用。另外，由于我们板子使用的 SPI 转 UART 串口模块挂到 SPI1 上，所以还要使能 SPI1 节点。如下：
