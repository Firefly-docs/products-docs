...
芯片本身带有一个内部看门狗。主板可另配一个外部看门狗，外部看门狗的常用型号为：PC9202
...
不同的外部看门狗型号，设备节点和时间可能不同。以 pc9202 为例：

看门狗的设备名称是`/dev/wdt_crl`，使用方法如下：
```bash
# 写入不同数字来开启看门狗并设置时间
...
3. 目前 <font color=red>iCore-3562JQ v1.0</font> 版本，外部看门狗开启后无法软件关闭，需要断电才能关闭
