...
AIO-1808-JD4 有灵活的启动方式。一般情况下，除非硬件损坏，AIO-1808-JD4 开发板是不会变砖的。

如果在升级过程中出现意外，bootloader 损坏，导致无法重新升级，此时仍可以进入 `MaskRom` 模式来修复。

## 加载方式

AIO-1808-JD4 有 32KB 的 BootRom 和 2MB 的内部 SRAM，支持从以下设备加载系统：

* SPI 接口
* eMMC 接口
* SDMMC 接口

另外 AIO-1808-JD4 支持从 USB OTG 接口下载系统代码。
...
* 双公头 USB 线连接好设备和主机，板端连接USB3.0 OTG口。
...
***要强行进入 MaskRom 模式，请参阅[《MaskRom》](upgrade_firmware.html)一章。***