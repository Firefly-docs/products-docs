...
## Core-3588JD4 显示接口的配置  

Core-3588JD4 默认只支持两种显示输出接口（但通过 164 Pin 接口可以扩展出 VGA，Display Port1 以及 DSI 等显示接口），分别是 HDMI 以及 Display Port，可以做到多屏同显/异显，接口图如下所示：  
...
Core-3588JD4 硬件上有两个 HDMI 显示输出接口：
...
Core-3588JD4 有一个 Display Port 显示输出接口，支持  DP TX 1.4a 协议，分辨率最高可以支持 7680x4320@30Hz。
...
## Core-3588JD4 多屏场景的配置  

由于 Core-3588JD4  默认只支持 3 路显示输出，HDMI0/HDMI1/Display Port0，有如下常规的场景配置：
