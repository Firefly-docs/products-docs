...
## Core-3588SJD4-AI 显示接口的配置  

Core-3588SJD4-AI 默认只支持两种显示输出接口（但通过 164 Pin 接口可以扩展出 VGA，Display Port1 以及 DSI 等显示接口），分别是 HDMI 以及 Display Port，可以做到多屏同显/异显，接口图如下所示：  
...
Core-3588SJD4-AI 硬件上有两个 HDMI 显示输出接口：
...
Core-3588SJD4-AI 有一个 Display Port 显示输出接口，支持  DP TX 1.4a 协议，分辨率最高可以支持 7680x4320@30Hz。
...
## Core-3588SJD4-AI 多屏场景的配置  

由于 Core-3588SJD4-AI  默认只支持 3 路显示输出，HDMI0/HDMI1/Display Port0，有如下常规的场景配置：
...
```
如果从 164 Pin 接口扩展出其他的显示接口（VGA，MIPI DSI 以及 Display Port1），那么相对应的配置场景可以参考：[ITX-3588J 多屏场景配置](https://wiki.t-firefly.com/zh_CN/Core-3588J/usage_display.html#itx-3588j-duo-ping-chang-jing-de-pei-zhi)
