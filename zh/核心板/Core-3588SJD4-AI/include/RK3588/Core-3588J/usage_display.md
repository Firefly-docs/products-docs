...
AIO-3588SJD4-AI 有四种显示输出接口，分别是 HDMI、Display Port、VGA 以及 MIPI DSI，可以做到多屏同显/异显，接口图如下所示：  
...
AIO-3588SJD4-AI 硬件上有两个 HDMI 显示输出接口：
...
AIO-3588SJD4-AI 有一个 VGA 显示输出接口，是采用 DP 转 VGA 芯片实现的，受限于 VGA 协议，分辨率最高可以支持 1080p@60Hz。  
...
结合 AIO-3588SJD4-AI 的 DSI0 接口和屏幕时序
...
* 四屏显示（接 VGA 屏幕，HDMI0 + HDMI1 + VGA + MIPI DSI0）  
```
&hdmi0_in_vp0 {
        status = "okay";
};
...
**以上两种配置下，HDMI0 不能接入分辨率大于 4096x2304@60Hz 的屏幕。**
