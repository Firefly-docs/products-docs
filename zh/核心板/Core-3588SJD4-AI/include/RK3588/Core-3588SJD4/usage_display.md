...
## AIO-3588SJD4-AI 显示接口的配置  

AIO-3588SJD4-AI 有三种显示输出接口，分别是 HDMI、Display Port 以及 MIPI DSI，可以做到多屏同显/异显，接口图如下所示：  
...
AIO-3588SJD4-AI 硬件上有一个 HDMI 显示输出接口：
...
AIO-3588SJD4-AI 有一个 Display Port 显示输出接口，支持  DP TX 1.4a 协议，分辨率最高可以支持 7680x4320@30Hz。
...
AIO-3588SJD4-AI 有两路 MIPI DSI 显示输出接口，均支持 DPHY2.0 和 4 Lane 的数据输出，最高可输出 4096x2304@60Hz（取决于连接的 Portx）。  

#### 软件配置

由于 DSI0 和 DSI1 在软件配置上类似，这里以 DSI1 的配置为例，外接的屏幕是 [Firefly V2 版本屏幕](https://wiki.t-firefly.com/DM-M10R800-V2/dm-m10r800-v2.html)，DSI0 软件上表示为 `dsi1`。

结合 AIO-3588SJD4-AI 的 DSI1 接口和屏幕时序
