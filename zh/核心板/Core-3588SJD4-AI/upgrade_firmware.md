...
本文介绍了如何将主机上的固件，通过Type-C 数据线烧录到 Core-3588SJD4-AI 开发板的存储器中。升级时，需要根据主机操作系统和固件类型来选择合适的升级方式。

## 准备工具
* AIO-3588SJD4-AI 开发板
* [固件](https://www.t-firefly.com/doc/download/323.html)
* 主机
* 良好的Type-C 数据线

## 准备固件
固件可以通过编译SDK获得，也可以通过[资源下载](https://www.t-firefly.com/doc/download/323.html)处下载公版固件（统一固件）。固件文件一般有两种：
...
下载 [Release_DriverAssistant.zip](https://www.t-firefly.com/doc/download/323.html#other_572)，解压，然后运行里面的 DriverInstall.exe 。为了所有设备都使用更新的驱动，请先选择`驱动卸载`，然后再选择`驱动安装`。
<center>
...
### Linux操作系统
Linux 下无须安装设备驱动
* [Linux_Upgrade_Tool](https://www.t-firefly.com/doc/download/323.html#other_571)工具

下载 [Linux_Upgrade_Tool](https://www.t-firefly.com/doc/download/323.html#other_571), 并按以下方法安装到系统中，方便调用：
...
AIO-3588SJD4-AI 硬件上没有设计 recovery 按键，因此没有硬件进入 loader 模式的方法，请使用软件方式。
#### 软件方式进入Loader模式
