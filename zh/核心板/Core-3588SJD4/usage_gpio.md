...
AIO-3588SJD4  开发板为了方便用户开发使用，引出了一排扩展的GPIO口供用户调试和开发,其对应引脚如下。
...
本文以 GPIO1_A4 和 GPIO1_B0 这两个 GPIO 口为例写一份简单操作 GPIO 口的驱动，在 SDK 的路径为 `kernel-5.10/drivers/gpio/gpio-firefly.c`,以下就以该驱动为例介绍 GPIO 的操作。


## GPIO引脚计算
Core-3588SJD4 有 5 组 GPIO bank：GPIO0~GPIO4，每组又以 A0~A7, B0~B7, C0~C7, D0~D7 作为编号区分，常用以下公式计算引脚：
...
```
下面演示GPIO1_B0 pin脚计算方法：

bank = 1;  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;//GPIO<font color=red>1</font>_B0 => 1,  bank ∈ [0,4]

group = 1;  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;//GPIO1_<font color=red>B</font>0 => 1,  group ∈ {(A=0), (B=1), (C=2), (D=3)}
...
GPIO1_B0 对应的设备树属性描述为:<&gpio1 8 GPIO_ACTIVE_HIGH>,由`kernel-5.10/include/dt-bindings/pinctrl/rockchip.h`的宏定义可知，也可以将GPIO1_B0描述为<&gpio1 RK_PB0 GPIO_ACTIVE_HIGH>。
```
#define RK_PA0		0
#define RK_PA1		1
#define RK_PA2		2
...
...
```
当GPIO1_B0脚没有被其它外设复用时, 我们可以通过export导出该引脚去使用
```shell
:/ # ls /sys/class/gpio/
...
gpio40  gpiochip128  gpiochip32   gpiochip64   unexport
:/ # ls /sys/class/gpio/gpio40
