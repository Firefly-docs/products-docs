...
可能是供电电流不足。请使用 12V/2A 或更高电流规格的稳定电源。

## 哪里有 RK3328 芯片技术手册？

请参阅 [Rockchip RK3328 TRM](https://t-firefly.oss-cn-hangzhou.aliyuncs.com/product/RK3328/Docs/TRM/Rockchip_RK3328TRM_V1.1-Part1-20170321.pdf)。

## 如何烧写 MAC 地址？

先按[升级固件](03-upgrade_firmware.md)中的方法进入升级模式，再使用 [WNpctool](https://pan.baidu.com/s/1kU727kF#list/path=%2F) 写入 MAC 地址。

## Ubuntu 插入耳机后没有声音怎么办？

打开 `Menu -> Multimedia -> PulseAudio Volume Control -> Configuration`，选择正在使用的声卡配置，并关闭未使用的声卡。

## Android 如何保存系统日志？

依次进入 `Settings -> System -> About tablet`，连续点击 `Build number` 5 次以开启开发者选项。返回上一级，进入 `Developer options` 并启用 `Enable logging to save`。系统会在存储根目录创建 `.LOGSAVE` 文件夹，其中包含 `logcat` 和内核 `kmsg` 日志。
...
* 安装RKDevInfoWriteTool
    * [下载地址](https://community.t-firefly.com/doc/download/62.html#other_379)
