...
AIO-3576JD4 有四种显示输出接口，分别是 HDMI、Display Port、 MIPI DSI 以及 LVDS，可以做到多屏同显/异显。
...
AIO-3576JD4 硬件上有一个 HDMI 显示输出接口：

* HDMI 支持 HDMI2.1 协议，分辨率最高可以支持 4k@120Hz

* HDMI 接口

![](../../../rk3576_img/Core-3576JD4/usage_display_hdmi_interface.jpg)
...
AIO-3576JD4 有一个 Display Port 显示输出接口，支持  DP TX 1.4a 协议，分辨率最高可以支持 4k@120Hz。

* DP 接口

![](../../../rk3576_img/Core-3576JD4/usage_display_dp_interface.jpg)
...
AIO-3576JD4 有一路 MIPI DSI 显示输出接口，支持 DPHY2.0 和 4 Lane 的数据输出。  
...
![](../../../rk3576_img/Core-3576JD4/usage_display_mipi_v2_interface.jpg)
...
AIO-3576JD4 有一路 LVDS 显示输出接口，它是由 TTL 转的 LVDS，支持最高输出 2K 分辨率。

#### 软件配置

固件默认适配的屏幕是 [Firefly DM-M10R800 屏幕](https://wiki.t-firefly.com/DM-M10R800/dm-m10r800.html)，

* LVDS 接口

![](../../../rk3576_img/Core-3576JD4/usage_display_lvds_interface.jpg)
...
这里要注意，RGB 不能连接在 vp0 上面，SDK 默认的软件配置是把 LVDS 连接在 vp1 上，这样会导致 DSI 不能连接 vp1 上。
...
AIO-3576JD4 硬件预留一个 eDP 显示输出接口 40Pin / 30Pin ( 默认硬件用作HDMI功能 )：
...
**这里要注意，eDP 和 HDMI 功能是复用关系，eDP 可选作 30Pin 或 40Pin 座子，没特殊需求情况下默认都是 HDMI 版本。**
