...
本文介绍了如何将主机上的固件，通过Type-C 数据线烧录到 Core-3588J 开发板的存储器中。升级时，需要根据主机操作系统和固件类型来选择合适的升级方式。

## 准备工具
* ITX-3588J  开发板
* [固件](https://community.t-firefly.com/doc/download/161)
* 主机
* 良好的Type-C 数据线

## 准备固件
固件可以通过编译SDK获得，也可以通过[资源下载](https://community.t-firefly.com/doc/download/161)处下载公版固件（统一固件）。固件文件一般有两种：
...
为避免由下载工具版本引起的烧写问题，推荐使用公版固件压缩包内部打包好的工具进行烧写，公版固件压缩包解压后如下：
```shell
...
```
也可以单独下载 [AndroidTool](https://community.t-firefly.com/doc/download/160)，解压，运行 `RKDevTool_Release_v2.xx` 目录里面的 `RKDevTool.exe`（注意，如果是 Windows 7/8,需要按鼠标右键，选择以管理员身份运行），如下图：
...
1. 设备断开电源
2. 按住 ITX-3588J  的 MaskRom 按键或者使用镊子短接 Core-3588J 上的两个测试点（均如下图所示）
3. 设备插入电源上电
此时设备就会进入 MaskRom 模式。



* 以下为 ITX-3588J  上相应的按键  
<center>
