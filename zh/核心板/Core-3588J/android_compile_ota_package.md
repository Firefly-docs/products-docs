...
在`rockdev/Image-rk3588_firefly_itx_3588j/` 目录下，找到对应的固件烧录到机器，此后机器就可以进行全量包升级和差分包升级，
把刚刚编译的完整包命名`rk3588_firefly_itx_3588j-ota-eng.xxx.zip` 为update.zip 放到/sdcard/目录下重启,并按照提示进行全量包升级。
...
参考[OTA 文档](https://community.t-firefly.com/doc/download/140.html)搭建服务器。