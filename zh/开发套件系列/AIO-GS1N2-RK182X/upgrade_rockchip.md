...
## 准备工具
* RK182X开发套件 开发板
* [固件](https://www.t-firefly.com/doc/download/358.html)
* 主机
* 良好的Type-A 数据线

## 准备固件
固件可以通过编译SDK获得，也可以通过[资源下载](https://www.t-firefly.com/doc/download/358.html)处下载公版固件（统一固件）。固件文件一般有两种：
...
下载 [Release_DriverAssistant.zip](https://www.t-firefly.com/doc/download/358.html#other_572)，解压，然后运行里面的 DriverInstall.exe 。为了所有设备都使用更新的驱动，请先选择`驱动卸载`，然后再选择`驱动安装`。
<center>
...
### Linux操作系统
Linux 下无须安装设备驱动
* [Linux_Upgrade_Tool](https://www.t-firefly.com/doc/download/358.html#other_571)工具

下载 [Linux_Upgrade_Tool](https://www.t-firefly.com/doc/download/358.html#other_571), 并按以下方法安装到系统中，方便调用：
...
* [Linux_adb_fastboot](https://www.t-firefly.com/doc/download/358.html)工具

下载 [Linux_adb_fastboot](https://www.t-firefly.com/doc/download/358.html), 并按以下方法安装到系统中，方便调用：
...
# 主模组 (Core-3588JD4/Core-3588SJD4 AI/Core-3576JD4)
## 前言

RK182X开发套件出厂默认安装Linux操作系统，如果用户要运行其他操作系统，需要使用对应的固件烧写到主板。

RK182X开发套件有灵活的启动方式。一般情况下，除非硬件损坏，RK182X开发套件 开发板是不会变砖的。
...
* [使用USB线缆升级固件](upgrade_firmware_rockchip.md)<br><br>使用Type-A 数据线将主板连接到电脑上，通过升级工具将固件烧写到主板上。<br><br>
* [使用SD卡升级固件](upgrade_firmware_sd_rockchip.md)<br><br>通过升级卡制作工具，将MicroSD卡制作为升级卡(建议使用32G及以下容量的卡)，将升级卡插入主板，上电开机，机器自动执行升级。


## 启动存储器

RK182X开发套件 从以下的存储器中加载系统：
...
***要强行进入 `MaskRom 模式`，请参阅[《MaskRom模式》](upgrade_maskrom_mode_rockchip.md)一章。***