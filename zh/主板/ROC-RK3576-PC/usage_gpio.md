...
本文以 GPIO0_C1  , GPIO0_C2 这个 GPIO 口为例写一份简单操作 GPIO 口的驱动，在 SDK 的路径为 `kernel/drivers/gpio/gpio-firefly.c`,以下就以该驱动为例介绍 GPIO 的操作。



## GPIO引脚计算
ROC-RK3576-PC 有 5 组 GPIO bank：GPIO0~GPIO4，每组又以 A0~A7, B0~B7, C0~C7, D0~D7 作为编号区分，常用以下公式计算引脚：
...
GPIO0_C1 对应的设备树属性描述为:<&gpio0 17 GPIO_ACTIVE_HIGH>,由`kernel/include/dt-bindings/pinctrl/rockchip.h`的宏定义可知，也可以将GPIO0_C1描述为<&gpio0 RK_PC1 GPIO_ACTIVE_HIGH>。
...
...
```
当GPIO0_C1 脚没有被其它外设复用时, 我们可以通过export导出该引脚去使用
```shell
:/ # ls /sys/class/gpio/
