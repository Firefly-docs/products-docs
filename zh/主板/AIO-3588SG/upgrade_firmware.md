...
本文介绍了如何将主机上的固件，通过Type-C 数据线烧录到 Core-3588SG 开发板的存储器中。升级时，需要根据主机操作系统和固件类型来选择合适的升级方式。

## 准备工具
* AIO-3588SG 开发板
* [固件](https://community.t-firefly.com/doc/download/201)
* 主机
* 良好的Type-C 数据线

## 准备固件
固件可以通过编译SDK获得，也可以通过[资源下载](https://community.t-firefly.com/doc/download/201)处下载公版固件（统一固件）。固件文件一般有两种：
...
下载 [Release_DriverAssistant.zip](https://community.t-firefly.com/doc/download/201)，解压，然后运行里面的 DriverInstall.exe 。为了所有设备都使用更新的驱动，请先选择`驱动卸载`，然后再选择`驱动安装`。
<center>
...
### Linux操作系统
Linux 下无须安装设备驱动
* [Linux_Upgrade_Tool](https://community.t-firefly.com/doc/download/201)工具

下载 [Linux_Upgrade_Tool](https://community.t-firefly.com/doc/download/201), 并按以下方法安装到系统中，方便调用：
...
* [Linux_adb_fastboot](https://community.t-firefly.com/doc/download/201)工具

下载 [Linux_adb_fastboot](https://community.t-firefly.com/doc/download/201), 并按以下方法安装到系统中，方便调用：
...
1. 设备断开电源
2. 按住 AIO-3588SG 的 MaskRom 按键或者使用镊子短接 Core-3588SG 上的两个测试点（均如下图所示）
3. 设备插入电源上电
此时设备就会进入 MaskRom 模式。



* 以下为 AIO-3588SG 上相应的按键  
<center>
