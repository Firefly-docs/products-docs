...
本文介绍了如何将主机上的固件，通过Type-C 数据线烧录到 ROC-RK3506J-CC 开发板的存储器中。升级时，需要根据主机操作系统和固件类型来选择合适的升级方式。

## 准备工具
* ROC-RK3506J-CC 开发板
* [固件](https://community.t-firefly.com/doc/download/309.html)
* 主机
* 良好的Type-C 数据线

## 准备固件
固件可以通过编译SDK获得，也可以通过[资源下载](https://community.t-firefly.com/doc/download/309.html)处下载公版固件（统一固件）。固件文件一般有两种：
...
下载 [Release_DriverAssistant.zip](https://community.t-firefly.com/doc/download/309.html)，解压，然后运行里面的 DriverInstall.exe 。为了所有设备都使用更新的驱动，请先选择`驱动卸载`，然后再选择`驱动安装`。
<center>
...
也可以单独下载 [AndroidTool](https://community.t-firefly.com/doc/download/309.html)，解压，运行 `RKDevTool_Release_v2.xx` 目录里面的 `RKDevTool.exe`（注意，如果是 Windows 7/8,需要按鼠标右键，选择以管理员身份运行），如下图：

![](../../../rk3506_img/common/upgrade_firmware_androidtool_zh.png)

### Linux操作系统
Linux 下无须安装设备驱动
* [Linux_Upgrade_Tool](https://community.t-firefly.com/doc/download/309.html)工具

下载 [Linux_Upgrade_Tool](https://community.t-firefly.com/doc/download/309.html), 并按以下方法安装到系统中，方便调用：
...
* [Linux_adb_fastboot](https://community.t-firefly.com/doc/download/139.html)工具

下载 [Linux_adb_fastboot](https://community.t-firefly.com/doc/download/139.html), 并按以下方法安装到系统中，方便调用：
...
![](../../../rk3506_img/ROC-RK3506J-CC/nor_flash-position.jpg)
