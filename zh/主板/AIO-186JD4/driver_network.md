...
AIO-186JD4 默认使能了这三种网络，取决于用户的使用，[各接口位置如图所示](interface_definition.md)  。

## 蜂窝网络

AIO-186JD4 只打开了 4G 的接口（因为 5G 网络硬件上和 USB3 座子有冲突，需要修改硬件去打开 5G 接口）。假若你做了修改，在硬件支持了 5G，也只能在 4G / 5G 接口之间去选择其中一个去使用。
接入步骤：
* SIM 卡
...
## 无线网络
AIO-186JD4 的无线网络接口为 PCIE M.2 接口。目前系统已编译并内置好的 ko 文件有：
```
