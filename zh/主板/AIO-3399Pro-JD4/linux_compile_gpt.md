...
[下载链接]: https://community.t-firefly.com/doc/download/61
[Firefly_Linux_SDK源码包]: https://community.t-firefly.com/doc/download/61#other_423
...
配置文件 `aio-rk3399pro-jd4-ubuntu.mk`:

```
./build.sh aio-rk3399pro-jd4-ubuntu.mk

#文件路径在 `device/rockchip/rk3399pro/aio-rk3399pro-jd4-ubuntu.mk`
```

如果配置文件生效会连接到 `device/rockchip/.BoardConfig.mk` ,检查该文件可以验证是否配置成功

**注意**: `aio-rk3399pro-jd4-ubuntu.mk` 为编译生成 Ubuntu 固件的配置文件。同时用户也可以通过参考该配置生成新的配置文件来适配自己所需要的固件。
...
# kernel defconfig
export LINUX_KERNEL_DEFCONFI=firefly3399pro_linux_defconfig   # 编译 kernel 配置文件

# kernel dts
export RK_KERNEL_DTS=rk3399pro-firefly-aioc                       # 编译 kernel 用到的 dts
...
*  [前往下载页面 下载对应的Ubuntu 根文件系统镜像](https://community.t-firefly.com/doc/download/61)
...
#修改aio-rk3399pro-jd4-ubuntu.mk文件
vim device/rockchip/rk3399pro/aio-rk3399pro-jd4-ubuntu.mk
...
**注意：** 打包前请确认 `tools/linux/Linux_Pack_Firmware/rockdev/package-file` 是否正确。打包会根据此文件进行分区打包。此文件链接会在 `./build.sh aio-rk3399pro-jd4-ubuntu.mk` 命令时更新，如果配置不对请返回`编译前配置`一节重新配置一次。
...
[Linux_SDK.7z]: https://community.t-firefly.com/doc/download/61
