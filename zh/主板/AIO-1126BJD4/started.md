## 介绍

**CORE-1126BJD4** 采用 Rockchip 四核 AI 视觉处理器 RV1126B，集成 3 TOPS NPU，支持轻量级 AI 大模型和深度学习框架。内置 12M ISP、8MP AI-ISP、集成多种算法加速器。支持 12M 视频编码，4K 视频解码，支持 4 个摄像头同时输入。采用 260P 金手指接口，尺寸小巧。广泛适用于人脸识别、闸机门禁、智能安防、智能网络摄像头等行业领域。

**CORE-1126BJD4** 可搭配不同的底板组合成不同形态的产品。分别可以组成 AIO-1126BJD4 套板和 AIO-1126BJD4V0 套板。

**CORE-1126BJD4** 正面：

![](../../../rv1126b_img/CORE-1126BJD4/CORE-1126BJD4-front.png)  

**CORE-1126BJD4** 背面：

![](../../../rv1126b_img/CORE-1126BJD4/CORE-1126BJD4-back.png)  

### AIO-1126BJD4

AIO-1126BJD4 开发板由核心板 CORE-1126BJD4 + 新底板 MB-JD4-RV1126B 组成。本维基介绍该套板的使用。

AIO-1126BJD4 正面：

![](../../../rv1126b_img/CORE-1126BJD4/AIO-1126BJD4-front.png)  

AIO-1126BJD4 背面：

![](../../../rv1126b_img/CORE-1126BJD4/AIO-1126BJD4-back.png)

### AIO-1126BJD4V0

AIO-1126BJD4V0 开发板由核心板 CORE-1126BJD4 + 旧底板 MB-JD4-RV11091126 组成。

点击跳转 AIO-1126BJD4V0 套板的维基教程：[AIO-1126BJD4V0 维基教程](../../主板/AIO-1126BJD4V0/started.md)

AIO-1126BJD4V0 正面：

![](../../../rv1126b_img/AIO-1126BJD4V0/AIO-1126BJD4V0-front.png)  

AIO-1126BJD4V0 背面：

![](../../../rv1126b_img/AIO-1126BJD4V0/AIO-1126BJD4V0-back.png)
 <a id="firmware-format"></a>