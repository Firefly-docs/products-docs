...
[firefly_rk3588_android14_git_20240822](https://community.t-firefly.com/doc/download/142.html)
...
下载页面选择云盘下载 [Android14.0 Bundle](https://community.t-firefly.com/doc/download/142.html)。
## ROC-RK3588S-PC 产品编译方法
...
Android14.0的`kernel.img`和`resource.img`包含在`boot.img`中，编译kernel后需要在android根目录下执行`./mkimage.sh`重新打包`boot.img`,然后烧写rockdev/Image-roc_rk3588s_pc/目录下的`boot.img`。
...
编译的时候执行 `./mkimage.sh` 会重新打包 `boot.img` 和 `super.img`, 并将其它相关的镜像文件拷贝到目录 rockdev/Image-roc_rk3588s_pc/ 中。以下列出一般固件用到的镜像文件：
