...
标准的 Linux 专门为 LED 设备定义了 LED 子系统。 在 ROC-RK3588S-PC开发板中的LED 均以设备的形式被定义。用户可以通过 `/sys/class/leds/` 目录控制LED。
...
在 `kernel/arch/arm64/boot/dts/rockchip/roc-rk3588s-pc.dtsi` 文件中定义 LED 节点，具体定义如下：
```
firefly_leds: leds {
...
--->LED Support
   --->LED Trigger support
      --->LED Timer Trigger
```
保存配置并编译内核，把 `kernel.img` 烧到 ROC-RK3588S-PC板子上 我们可以使用串口输入命令，就可以看到蓝灯不停的间隔闪烁。
