...
AIO-3588JQ  开发板为了方便用户开发使用，引出了一排扩展的GPIO口供用户调试和开发，其对应引脚如下。
...
本文以 GPIO2_C3 和 GPIO3_A6 这两个 GPIO 口为例写一份简单操作 GPIO 口的驱动，在 SDK 的路径为 `kernel-5.10/drivers/gpio/gpio-firefly.c`,以下就以该驱动为例介绍 GPIO 的操作。


## GPIO引脚计算
iCore-3588JQ 有 5 组 GPIO bank：GPIO0~GPIO4，每组又以 A0~A7, B0~B7, C0~C7, D0~D7 作为编号区分，常用以下公式计算引脚：
...
GPIO2_C3 对应的设备树属性描述为:<&gpio2 19 GPIO_ACTIVE_HIGH>,由`kernel-5.10/include/dt-bindings/pinctrl/rockchip.h`的宏定义可知，也可以将GPIO1_D0描述为<&gpio2 RK_PC3 GPIO_ACTIVE_HIGH>。
...
...
```
当GPIO2_C3脚没有被其它外设复用时, 我们可以通过export导出该引脚去使用
```shell
:/ # ls /sys/class/gpio/
