...
Firefly-RK3399 开发板默认外置支持了两个LCD屏接口，一个是MIPI（兼容10.1寸以及8寸），一个是EDP，接口对应板子上的位置如下图：

10.1 寸 MIPI
![](../../../rk3399_img/Firefly-RK3399/pc+_mipi101_1.jpg)

8寸 MIPI
![](../../../rk3399_img/Firefly-RK3399/panel_mipi8.jpg)
...
### 引脚配置
###### MIPI屏
Firefly-RK3399 MIPI DSI文件路径 
适配8寸屏幕 : kernel/arch/arm64/boot/dts/rockchip/rk3399-roc-pc-plus-mipi8.dts 

从文件rk3399-roc-pc-plus-mipi8.dts我们可以看到以下语句:
...
Firefly-RK3399 开发板外置了一个背光接口用来控制屏幕背光。

在DTS文件：kernel/arch/arm64/boot/dts/rockchip/rk3399-roc-pc-plus-mipi8.dts中配置了背光信息，如下：
```
......
...
与EDP屏不同，MIPI屏的 Timing 写在DTS文件中，在kernel/arch/arm64/boot/dts/rockchip/rk3399-roc-pc-plus-mipi8.dts中可以看到以下语句：
...
mipi屏上完电后需要发送初始化指令才能使之工作，可以在kernel/arch/arm64/boot/dts/rockchip/rk3399-roc-pc-plus-mipi8.dts中可以看到mipi的初始化指令列表：
```
&dsi {    
            status = "okay";      
        ...
...
### 引脚配置
###### EDP屏
Firefly-RK3399 的SDK有EDP DSI的DTS文件：kernel/arch/arm64/boot/dts/rockchip/rk3399-roc-pc-plus-edp.dts，从该文件中我们可以看到以下语句：
```
