...
本文介绍了如何将主机上的固件，通过USB数据线烧录到 Firefly-RK3399 开发板的存储器中。升级时，需要根据主机操作系统和固件类型来选择合适的升级方式。
...
## 准备固件
固件可以通过编译 SDK 获得，也可以通过[资源下载](https://community.t-firefly.com/doc/download/3)处下载公版固件(统一固件)。固件文件一般有两种：
...
下载 [Release_DriverAssistant.zip](https://community.t-firefly.com/doc/download/3#other_11)，解压，然后运行里面的 DriverInstall.exe 。为了所有设备都使用更新的驱动，请先选择`驱动卸载`，然后再选择`驱动安装`。
...
* 下载并运行[AndroidTool](https://community.t-firefly.com/doc/download/3#other_248)的RKDevTool.exe
...
* [Linux_Upgrade_Tool](https://community.t-firefly.com/doc/download/3#windows_375)工具

**<font color=#ff0000 >注意</font>**:不同固件使用的工具版本可能不同,请根据[《使用USB线烧写须知(重要)》]下载对应的版本

下载 [Linux_Upgrade_Tool](https://community.t-firefly.com/doc/download/3#windows_375), 并按以下方法安装到系统中，方便调用：
...
[RKDevTool]: https://community.t-firefly.com/doc/download/3#other_248