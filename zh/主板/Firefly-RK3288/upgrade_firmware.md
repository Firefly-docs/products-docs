...
1. 设备断开所有电源。
1. 拔出 SD 卡。
1. 用Micro USB 数据线连接好设备和主机。
1. 用金属镊子接通Firefly-RK3288上的如下图所示的两个测试点并保持(如下图所示)。
1. 设备插入电源。
1. 稍候片刻，之后松开镊子。
...
本文介绍了如何将主机上的固件，通过Micro USB 数据线烧录到 Firefly-RK3288 开发板的闪存中。升级时，需要根据主机操作系统和固件类型来选择合适的升级方式。
