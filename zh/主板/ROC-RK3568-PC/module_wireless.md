...
确保固件为最新[公版固件](https://community.t-firefly.com/doc/download/94.html)，固件默认关闭GPS功能，需要手动使能，以下是修改方法：
...
* 使能GPS
    * 修改SDK目录下`device/rockchip/rk356x/rk3568_firefly_roc_pc/rk3568_firefly_roc_pc.mk`里面`BOARD_HAS_GPS`参数为`true`，实现固件使能GPS功能，修改后重新编译SDK并烧录固件生效。

* 修改串口配置(串口节点或者波特率)
    * 将SDK目录下`device/rockchip/rk356x/rk3568_firefly_roc_pc/gps/u-blox.conf`里面`SERIAL_DEVICE`或者`SERIAL_BAUD_RATE`修改成对应模组配置的值，修改后重新编译SDK并烧录固件生效