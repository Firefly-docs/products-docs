...
AIBOX-PRO-KIT出厂默认安装Linux操作系统，如果用户要运行其他操作系统，需要使用对应的固件烧写到主板。

AIBOX-PRO-KIT有灵活的启动方式。一般情况下，除非硬件损坏，AIBOX-PRO-KIT 开发板是不会变砖的。
