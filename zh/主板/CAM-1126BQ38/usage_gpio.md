...
## GPIO引脚计算
CAM-1126BQ38 有 8 组 GPIO bank：GPIO0~GPIO7，每组又以 `A0~A7`, `B0~B7`, `C0~C7`, `D0~D7` 作为编号区分，常用以下公式计算引脚：
...
GPIO 调试有一个很好用的工具，那就是 IO 指令，CAM-1126BQ38 的 Linux 系统默认已经内置了 IO 指令，使用 IO 指令可以实时读取或写入每个 IO 口的状态，这里简单介绍 IO 指令的使用。首先查看 IO 指令的帮助：
