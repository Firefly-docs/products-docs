...
![](../../../rk3399_img/AIO-3399J/module_wireless_usb.jpg)
...
公版固件默认支持 EC20 4G 模组 [[链接]](https://community.t-firefly.com/doc/download/119.html)
...
公版固件默认支持 EC200 4G 模组 [[链接]](https://community.t-firefly.com/doc/download/119.html)
...
* 将模组的VCC、GND、TX、RX分别接到 AIO-3399J **UART2**(<font color = "red">对应节点为`/dev/ttysWK2`</font>)的3.3V、GND、RX、TX，注意避免接错VCC、GND、TX、RX<font color = "red">导致烧坏模组</font>
...
公版新固件默认支持GPS模组, 但需要手动打开。 或直接下载GPS默认启动固件[百度云](https://community.t-firefly.com/doc/download/132.html)
...
确保固件为最新[公版固件](https://community.t-firefly.com/doc/download/31.html)，固件默认关闭GPS功能，需要手动使能，以下是修改方法：
...
* 使能GPS
    * 修改SDK目录下`device/rockchip/rk3399/rk3399_firefly_aio.mk`(Android7.1以上的版本为`device/rockchip/rk3399/rk3399_firefly_aio/rk3399_firefly_aio.mk`)里面`BOARD_HAS_GPS`参数为`true`，实现固件使能GPS功能，修改后重新编译SDK并烧录固件生效。

* 修改串口配置(串口节点或者波特率)
    * 将SDK目录下`device/rockchip/rk3399/gps/u-blox.conf`(Android7.1以上的版本为`device/rockchip/rk3399/rk3399_firefly_aio/gps/u-blox.conf`)里面`SERIAL_DEVICE`或者`SERIAL_BAUD_RATE`修改成对应模组配置的值，修改后重新编译SDK并烧录固件生效
