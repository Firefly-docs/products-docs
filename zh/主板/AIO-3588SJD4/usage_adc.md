...
AIO-3588SJD4  开发板上的 AD 接口有两种，分别为：温度传感器 (Temperature Sensor)、逐次逼近ADC (Successive Approximation Register)。其中：
...
AIO-3588SJD4  SAR-ADC 的 DTS 节点在 `kernel-5.10/arch/arm64/boot/dts/rockchip/rk3588s.dtsi` 文件中定义，如下所示：
...
* 这里申请的是SARADC通道6，只是作为示例，`仅供参考，最终以实际硬件接口为准`。

### 在驱动文件中匹配 DTS 节点

用户驱动可参考 Firefly adc demo :`kernel-5.10/drivers/iio/adc/adc-firefly-demo.c`，这是一个侦测 AIO-3588SJD4  风扇状态的驱动。首先在驱动文件中定义 `of_device_id` 结构体数组：
...
编译内核，烧录内核到 AIO-3588SJD4  开发板上，然后插拔风扇时，会打印内核 log 信息如下：
