...
SPI 是一种高速的，全双工，同步串行通信接口，用于连接微控制器、传感器、存储设备等。 AIO-3588SJD4  开发板提供了 SPI0接口，具体位置如下图：
...
AIO-3588SJD4 与 XM25QU128C 硬件连接如下表：
...
* spi-max-frequency：此处设置 spi 使用的最高频率，AIO-3588SJD4 最高支持 50000000。
...
Linux 提供了一个功能有限的 SPI 用户接口，如果不需要用到 IRQ 或者其他内核驱动接口，可以考虑使用接口 `spidev` 编写用户层程序控制 SPI 设备。在 AIO-3588SJD4 开发板中对应的路径为： `/dev/spidev1.0`
