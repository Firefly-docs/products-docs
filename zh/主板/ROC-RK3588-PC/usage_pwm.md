...
ROC-RK3588-PC的 PWM 驱动为：`kernel-5.10/drivers/pwm/pwm-rockchip.c`
...
```
pwms参数分析：
`&pwm15`: 在 rk3588s.dtsi 中有定义。
`0` ：表示 chip index ,一般是0,因为rockchip pwm每个chip只有一个。
`100000000` ： 一个周期时间为100000000纳秒 ，一秒有10个100000000纳秒 ，所以这个pwm输出周期为10Hz。
`1`: 表示极性， 0为正常极性，1为反转极性。

`duty_ns`表示占空比激活时间，单位也是纳秒

由于 pwm15 默认用作 FAN 控制，所以在上述的修改基础上，还需要关闭 FAN 功能，才可以使能 pwm demo：  
