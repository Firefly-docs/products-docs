...
执行完成后会生成编译输出目录，`output/firefly_rk3588`，后续也可以在该目录下执行 `make` 相关操作。
...
将修改保存到配置文件 `configs/firefly_rk3588_defconfig`。
...
* 解压
  ...
  软件包会解压在 `output/firefly_rk3588/build/<package>-<version>` 目录下。
...
* 安装
  ...
  编译完成后，会将需要的编译生成文件拷贝到 `output/firefly_rk3588/target/` 目录。
...
编译完成后，在编译输出目录 `output/firefly_rk3588` 会生成子目录，说明如下：
...
Buildroot 编译完成后，会在 `output/firefly_rk3588/host/` 目录下，生成交叉编译工具，我们可以用来编译目标程序。
...
将编译好的根文件系统 `output/firefly_rk3588/images/rootfs.ext2` 下载到设备。启动设备，可以看到已添加文件 `/etc/overlay-test`。
