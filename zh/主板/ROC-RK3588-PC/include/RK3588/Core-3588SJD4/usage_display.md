...
## ROC-RK3588-PC 显示接口的配置  

ROC-RK3588-PC 有三种显示输出接口，分别是 HDMI、Display Port 以及 MIPI DSI，可以做到多屏同显/异显，接口图如下所示：  
...
ROC-RK3588-PC 硬件上有一个 HDMI 显示输出接口：
...
ROC-RK3588-PC 有两路 MIPI DSI 显示输出接口，均支持 DPHY2.0 和 4 Lane 的数据输出，最高可输出 4096x2304@60Hz（取决于连接的 Portx）。  
...
结合 ROC-RK3588-PC 的 DSI1 接口和屏幕时序
