...
EC-A3576C 整机未引出调试串口接口，需要拆开主机后，使用外接 USB 转 TTL 串口模块连接主板上的调试串口排针进行调试。

调试串口的连接方式与使用方法详见：[调试串口](usb_to_ttl.md)。

## CAN

AIO-3576C 提供 1 路 CAN 接口（凤凰端子座）。接线时 CAN_H 接 CAN_H，CAN_L 接 CAN_L。
...
AIO-3576C 提供 1 个 RS232 接口和 1 个 RS485 接口（均为凤凰端子座）。
...
AIO-3576C 的 SIM 卡槽用于配合 4G/5G 模块实现移动网络连接。**4G/5G 模块为选配**，需要在机箱内部安装 4G/5G 模块后，SIM 卡功能才能正常使用。SIM 卡插入方向如下图所示，插拔前请先断电。
...
AIO-3576C 提供 1 个 HDMI 显示输出接口，支持 HDMI2.1 协议，分辨率最高可以支持 4K@120Hz。
...
AIO-3576C 提供 2 个 RJ45 千兆网口，对应系统中的 `eth0`、`eth1` 两个设备。
