...
获取安装包后需将其传输至 EC-A1684JD4 FD 上，并执行以下命令进行安装：
...
获取模型文件后需将其传输至 EC-A1684JD4 FD 上，并执行以下命令进行导入：
...
将部署包传输至 EC-A1684JD4 FD 机器，推荐使用 `scp` 命令传输：

1. 在 EC-A1684JD4 FD 上执行 `ifconfig` 命令查看其 IP 地址。
2. 输入 `scp` 命令传输部署包，例：`scp llama3.tar linaro@<aibox-ip-addr>:~/`，其中 `<aibox-ip-addr>` 为 EC-A1684JD4 FD 的 IP 地址。
...
[下载中心]: https://community.t-firefly.com/doc/download/212