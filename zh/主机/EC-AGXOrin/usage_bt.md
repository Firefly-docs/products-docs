...
EC-AGXOrin 支持无线蓝牙，可以通过 `hciconfig -a` 命令显示蓝牙设备信息：
...
（5）此时在智能手机就能发现 EC-AGXOrin 蓝牙设备，手机点击后就能配对上去：
...
EC-AGXOrin 默认没有安装该工具，用户需要自行编译安装，这里演示 1.3.0 版本的步骤，如下：
...
在 Linux 中需要用到 Obex 服务，首先 EC-AGXOrin 根据前面的步骤连接好蓝牙设备，然后开启 Obex 守护进程，并设置接收的目录为 `/home/linaro/`：
...
（5）查看 EC-AGXOrin 的 `/home/linaro/`目录有 `test.txt` 文件：
