...
ROC-RK3566-PC的SDK下`kernel/arch/arm64/configs/firefly_defconfig`已经把LCD相关的配置设置好了，如果自己做了修改，请注意把以下配置加上：
```
...
ROC-RK3566-PC的SDK有MIPI DSI的DTS文件：
...
在硬件信号上LCD_EN引脚高电平有效，而LCD_RST引脚是低电平有效，具体的引脚配置请参考[《GPIO 使用》](driver_gpio.html)一节。需要注意的是ROC-RK3566-PC的dsi1中，LCD_EN:(PCA_IO0_7)使用的是PCA9555扩展芯片引出来的gpio，需要PCA9555 驱动起来以后才能控制该gpio。
### 背光配置
...
## EDP DTS配置
### 引脚配置
ROC-RK3566-PC的SDK有EDP的DTS文件：`kernel/arch/arm64/boot/dts/rockchip/rk3568-firefly-aioj-edp_M156X40.dts`，从该文件中我们可以看到以下语句：
```
