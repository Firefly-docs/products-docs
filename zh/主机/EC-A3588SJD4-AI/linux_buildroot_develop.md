...
执行完成后会生成编译输出目录，`output/rockchip_rk3588_defconfig`，后续也可以在该目录下执行 `make` 相关操作。
...
将修改保存到配置文件 `configs/rockchip_rk3588_defconfig_defconfig`。
...
* 解压
  ...
  软件包会解压在 `output/rockchip_rk3588_defconfig/build/<package>-<version>` 目录下。
...
* 安装
  ...
  编译完成后，会将需要的编译生成文件拷贝到 `output/rockchip_rk3588_defconfig/target/` 目录。
...
编译完成后，在编译输出目录 `output/rockchip_rk3588_defconfig` 会生成子目录，说明如下：
...
Buildroot 编译完成后，会在 `output/rockchip_rk3588_defconfig/host/` 目录下，生成交叉编译工具，我们可以用来编译目标程序。
...
打开配置菜单 `make menuconfig`，通过设置 `BR2_ROOTFS_OVERLAY` 选项，添加用于覆盖的根目录。对于 RK3588，默认已添加了目录 `board/rockchip/rk356x/fs-overlay/`。
...
将编译好的根文件系统 `output/rockchip_rk3588_defconfig/images/rootfs.ext2` 下载到设备。启动设备，可以看到已添加文件 `/etc/overlay-test`。
