...
AIBOX-Orin NX 板载 Type-C Console 调试串口，无需外接串口模块，使用 Type-C 数据线连接主机与 PC 即可进行串口调试。

## 登录

AIBOX-Orin NX 登录方式有两种，一种是通过 HDMI 登录，一种是通过 Console（调试串口）进行终端登录。
...
AIBOX-Orin NX 的外部看门狗设备名称是`/dev/wdt_crl`，使用方法如下:
...
AIBOX-Orin NX 有一路外部的 RTC，由外部的底板电容进行供电，掉电后短时间内保证RTC运行。在 kernel 中表示为 `rtc0`。
