...
EC-A1684XJD4 FD 的内存有一部分被分配于 NPU、VPP 和 VPU 设备，因此使用 `free -h` 等指令获取的内存数据与 EC-A1684XJD4 FD 的实际内存不一致。

如需查看详细的内存分配设置，或者默认内存分配与实际需求冲突，需调整内存分配的，可参照以下文档进行相应操作。

## 安装内存分配工具

从[下载中心]中获取内存分配工具压缩包，并将其传输到 EC-A1684XJD4 FD 上，执行以下命令解压压缩包：
...
[下载中心]: https://www.t-firefly.com/doc/download/208.html