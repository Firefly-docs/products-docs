...
AIBOX PRO 登录方式有两种，一种是通过 Console（调试串口）进行终端登录，一种是通过 HDMI 登录。
...
AIBOX PRO 的 SIM 卡槽用于配合 4G 模块实现移动网络连接。**4G 模块为选配**，需要在机箱内部安装 4G 模块后，SIM 卡功能才能正常使用。SIM 卡插入方向如下图所示，插拔前请先断电。
