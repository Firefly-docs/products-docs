...
ROC-RK3399-PC-PLUS开发板默认外置支持了两个LCD屏接口，一个是LVDS，一个是EDP，接口对应板子上的位置如下图：
...
ROC-RK3399-PC-PLUS的SDK有LVDS DSI的DTS文件：kernel/arch/arm64/boot/dts/rockchip/rk3399pro-firefly-aioc-lvds-HSX101H40C.dts，从该文件中我们可以看到以下语句：
```
/ {
...
ROC-RK3399-PC-PLUS开发板外置了一个背光接口用来控制屏幕背光，如下图所示：
...
```
pwms属性：配置PWM，范例里面默认使用pwm0，25000ns是周期(40 KHz)。LVDS需要加背光电源控制脚，在kernel/arch/arm64/boot/dts/rockchip/rk3399pro-firefly-aioc-lvds-HSX101H40C.dts中可以看到以下语句：
```
&backlight {
    status = "okay";
...
与EDP屏不同，LVDS屏的 Timing 写在DTS文件中，在kernel/arch/arm64/boot/dts/rockchip/rk3399pro-firefly-aioc-lvds-HSX101H40C.dts中可以看到以下语句：
...
### DTS配置
#### 引脚配置
ROC-RK3399-PC-PLUS的SDK有EDP DSI的DTS文件：kernel/arch/arm64/boot/dts/rockchip/rk3399pro-firefly-aioc-edp.dts，从该文件中我们可以看到以下语句：
```
{
        panel: panel {
