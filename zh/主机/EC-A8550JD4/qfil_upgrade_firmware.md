...
本文介绍了如何将固件，通过 Type-A 转 C USB 数据线 烧录到 EC-A8550JD4 开发板的存储器中。
...
### 准备固件
固件可以通过编译SDK获得，也可以通过[资源下载](https://community.t-firefly.com/doc/download/371)处下载公版固件（完整固件）。
...
前往 [USB 驱动](https://community.t-firefly.com/doc/download/371) 下载驱动。
...
前往 [QTSP 工具](https://community.t-firefly.com/doc/download/371) 下载工具。
...
前往 [Platform Tools](https://community.t-firefly.com/doc/download/371) 下载工具。
...
EC-A8550JD4 整机进入 EDL 模式的硬件操作方式待补充。

在设备可正常运行的情况下，推荐使用软件方式：使用 Type-A 转 C USB 数据线 连接好设备的烧录口和电脑，然后在设备终端或调试串口上运行 `sudo systemctl reboot edl` 进入 EDL 升级模式。
### 软件方式
