...
EC-A1684JD4 FD 登录方式有两种，一种是通过 RS232，一种是通过以网络 SSH。
...
EC-A1684JD4 FD  拥有两个以太网接口，默认出厂时网口 0 （靠近串口）设置了动态 IP，而网口 1 （靠近 HDMI 口）设置了静态 IP `192.168.150.1`，子网掩码 `255.255.255.0`，可以将 PC 设置成 `192.168.150.2/24` 来做初次登录。
