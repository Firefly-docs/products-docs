...
本文介绍了如何将主机上的固件，通过Type-C 数据线烧录到 Station-M2 开发板的存储器中。升级时，需要根据主机操作系统和固件类型来选择合适的升级方式。
...
1. 用金属镊子接通Station-M2上的如下图所示的两个测试点并保持(如下图所示)。
1. 设备插入电源。
1. 稍候片刻，之后松开镊子。
...
1. 设备断开电源
2. 用针状物捅住 Station-M2 上的 MaskRom 孔 并保持
3. 设备插入电源上电
此时设备进入 MaskRom 模式
