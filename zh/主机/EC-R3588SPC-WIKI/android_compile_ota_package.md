...
#### 生成完整包路径
```
out/target/product/roc_rk3588s_pc/roc_rk3588s_pc-ota-eng.xxx.zip 
```
#### 生成基础素材包路径
```
out/target/product/roc_rk3588s_pc/obj/PACKAGING/target_files_intermediates/roc_rk3588s_pc-target_files-eng.xxx.zip
```
...
在`rockdev/Image-roc_rk3588s_pc/` 目录下，找到对应的固件烧录到机器，此后机器就可以进行全量包升级和差分包升级，
把刚刚编译的完整包命名`roc_rk3588s_pc-ota-eng.xxx.zip` 为update.zip 放到/sdcard/目录下重启,并按照提示进行全量包升级。
...
参考[OTA 文档](https://www.t-firefly.com/doc/download/182.html#other_638)搭建服务器。