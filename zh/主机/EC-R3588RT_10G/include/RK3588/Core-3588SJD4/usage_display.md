...
## EC-R3588RT_10G 显示接口的配置  

EC-R3588RT_10G 有三种显示输出接口，分别是 HDMI、Display Port 以及 MIPI DSI，可以做到多屏同显/异显，接口图如下所示：  
...
EC-R3588RT_10G 硬件上有一个 HDMI 显示输出接口：
...
EC-R3588RT_10G 有一个 Display Port 显示输出接口，支持  DP TX 1.4a 协议，分辨率最高可以支持 7680x4320@30Hz。
...
EC-R3588RT_10G 有两路 MIPI DSI 显示输出接口，均支持 DPHY2.0 和 4 Lane 的数据输出，最高可输出 4096x2304@60Hz（取决于连接的 Portx）。  
...
结合 EC-R3588RT_10G 的 DSI1 接口和屏幕时序
