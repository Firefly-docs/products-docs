...
EC-R3588RT_10G 有四种显示输出接口，分别是 HDMI、Display Port、VGA 以及 MIPI DSI，可以做到多屏同显/异显，接口图如下所示：  
...
EC-R3588RT_10G 硬件上有两个 HDMI 显示输出接口：
...
EC-R3588RT_10G 有一个 VGA 显示输出接口，是采用 DP 转 VGA 芯片实现的，受限于 VGA 协议，分辨率最高可以支持 1080p@60Hz。  
...
结合 EC-R3588RT_10G 的 DSI0 接口和屏幕时序
