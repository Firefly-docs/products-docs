...
EC-R3328PC 与 W25Q128FV 硬件连接可参考下表：
...
* spi-max-frequency：此处设置 spi 使用的最高频率。EC-R3328PC 最高支持 48000000。
...
Linux 提供了一个功能有限的 SPI 用户接口，如果不需要用到 IRQ 或者其他内核驱动接口，可以考虑使用接口 `spidev` 编写用户层程序控制 SPI 设备。在 EC-R3328PC 开发板中对应的路径为： `/dev/spidev0.0`
