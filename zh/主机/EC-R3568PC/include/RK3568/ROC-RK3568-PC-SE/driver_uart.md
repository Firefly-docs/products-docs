...
ROC-RK3568-PCSE支持UART、RS232、RS485接口，分别为双扩展接口上的UART7、集成在RJ45接口的两路RS232和一路RS485，其串口接口图如下:
...
### UART7
UART7的TX、RX引脚存在复用的情况，实际`UART7 TX`和`UART7 RX` 分别对应着开发板上的丝印`PWM14`、`SPDIF`，公版固件默认UART7是打开的，使用做其他功能需先关闭UART7，以下是复用详情：
...
RS232_1、RS232_2和RS485分别从主控的UART2、UART3、UART4转换而来，其中由于UART2默认作为DEBUG串口，RS232_1无法直接使用，需要将[UART2配置为普通串口](#ru-he-jiang-uart2-pei-zhi-wei-pu-tong-chuan-kou)后才可使用。相关的硬件连接及定义可以参考[ROC-RK3568-PCSE原理图](https://www.t-firefly.com/doc/download/180.html#other_648)，以下是RJ45接口的部分PIN脚定义：
