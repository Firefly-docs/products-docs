...
EC-A186JD4 登录方式有两种，一种是通过 Type-C，一种是通过以网络 SSH。

## Type-C

通过 Type-C 登录方式，用户可以直接参考 AIO-186JD4 维基页面的《[使用 Type-C 串口调试](https://wiki.t-firefly.com/Core-186JD4/debug.html#shi-yong-type-c-chuan-kou-tiao-shi)》部分，登录账号与密码均为 `linaro`。

## 以太网 SSH

EC-A186JD4  拥有两个以太网接口，默认出厂时网口 0 （靠近 USB 座子）设置了动态 IP，而网口 1 （靠近 HDMI 口）设置了静态 IP `192.168.150.1`，子网掩码 `255.255.255.0`，可以将 PC 设置成 `192.168.150.2/24` 来做初次登录。
