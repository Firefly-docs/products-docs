...
EC-A1688JD4 板载 Type-C 调试串口，使用 Type-C 转 USB 数据线连接机器的 Type-C 口和 PC 的 USB 口即可直连调试。
...
EC-A1688JD4 提供 1 路 HDMI 输出接口，支持标准 HDMI 2.0 输出。
...
EC-A1688JD4 板载 RS232 串口对应设备节点 `/dev/ttyS2`，默认波特率 `115200`。
...
除通过 Type-C 调试串口登录（见上文调试串口一节）外，EC-A1688JD4 还提供 2 个千兆以太网口，可以通过以太网 SSH 远程登录。

### 以太网 SSH 登录

EC-A1688JD4 默认出厂时网口 0（靠近 USB 座子）设置了动态 IP，而网口 1（靠近 HDMI 口）设置了静态 IP `192.168.150.1`，子网掩码 `255.255.255.0`，可以将 PC 设置成 `192.168.150.2/24` 来做初次登录。
...
若登录失败，可以尝试使用 PC 机是否可以 `ping` 通 EC-A1688JD4 网口 1 的 IP 地址，注意的是 PC 机要添加同网段 IP，原因是 PC 机的 IP 地址不一定处于同一网段。
...
EC-A1688JD4 提供 TF 卡座。TF 卡的挂载方法与 U 盘类似：
...
EC-A1688JD4 的 SIM 卡槽用于配合 4G 模块实现移动网络连接。**4G 模块为选配**，需要在机箱内部安装 4G 模块后，SIM 卡功能才能正常使用。SIM 卡插入方向如下图所示，插拔前请先断电。
...
EC-A1688JD4 提供 NVME 接口（PCIE3.0 x 1）。一般可挂载的 PCIE 节点为：`/dev/nvme0n1p1`。
...
EC-A1688JD4 的内存有一部分被分配于 NPU、VPP 和 VPU 设备，因此使用 `free -h` 等指令获取的内存数据与机器的实际内存不一致。

如需查看详细的内存分配设置，或者默认内存分配与实际需求冲突，需调整内存分配的，可参照以下步骤进行相应操作。

### 安装内存分配工具

从 [下载中心](https://community.t-firefly.com/doc/download/250) 获取内存分配工具压缩包，并将其传输到 EC-A1688JD4 上，执行以下命令解压压缩包：
...
EC-A1688JD4 有一个 RS485 接口，设备名称为 `/dev/ttyS4`，支持半双工，默认波特率为 `9600`。
...
将 EC-A1688JD4 的 RS485 的 A、B、GND 引脚分别和主机串口适配器（USB 转 485 转串口模块）的 A、B、GND 引脚相连。
...
EC-A1688JD4 的 CAN 接口位置参见页首整机图。由于只有一个 CAN，所以默认在内核中，第一个创建的设备为 `can0`。
