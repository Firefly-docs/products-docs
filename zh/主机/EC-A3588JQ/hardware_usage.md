...
EC-A3588JQ 整机未引出调试串口接口，需要拆开主机后接入核心板上的调试串口，并外接 USB 转 TTL 串口模块进行调试。

调试串口的连接方式与使用方法详见：[调试串口](usb_to_ttl.md)。

## CAN

EC-A3588JQ 提供 1 路 CAN 接口。接线时 CAN_H 接 CAN_H，CAN_L 接 CAN_L。
...
EC-A3588JQ 提供 2 个 RS232 接口（RS232_0、RS232_1）和 1 个 RS485 接口。其中 RS232_0 由主控 UART0 扩展，RS232_1 由主控 UART5 扩展，RS485 由主控 UART1 扩展。推荐使用官方 FC10 转 DB9 串口线，不同厂商的串口线线序可能不同，会导致串口无法通信。
...
* MIPI-DSI x2：两路 MIPI DSI 输出接口，均支持 DPHY2.0 和 4 Lane 数据输出，最高可输出 4096x2304@60Hz（取决于连接的 Portx）。
* EDP：EDP 输出接口，可点 EDP 屏（支持屏幕以实际系统为准）。
...
多屏使用注意事项：RK3588 做 8K 输出时会同时占用 Port0 与 Port1 的资源。SDK 默认配置为 HDMI0（`hdmi0_in_vp0`）+ DP0（`dp0_in_vp2`），此配置下 HDMI2.1（8K）与 Display Port（4K@60Hz）可以同时正常输出；请勿将 DP0 改配到 vp1，否则 HDMI0 8K 输出时 DP0 显示会异常。
...
EC-A3588JQ 提供 2 个 RJ45 千兆网口，对应系统中的 `eth0`、`eth1` 两个设备。
...
EC-A3588JQ 提供 1 个 TF Card 卡槽，插入 TF 卡后系统会自动识别挂载（挂载路径以实际系统为准）。

## SIM 卡

EC-A3588JQ 的 SIM 卡槽用于配合 4G/5G 模块实现移动网络连接。**4G/5G 模块为选配**，需要在机箱内部安装 4G/5G 模块后，SIM 卡功能才能正常使用。SIM 卡插入方向如下图所示，插拔前请先断电。
