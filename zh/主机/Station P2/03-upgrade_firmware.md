...
本文介绍了如何将主机上的固件，通过Type-C 数据线烧录到 Station-P2 开发板的存储器中。升级时，需要根据主机操作系统和固件类型来选择合适的升级方式。
...
## 准备固件
固件可以通过编译SDK获得，也可以通过[资源下载](https://community.t-firefly.com/doc/download/217)处下载公版固件（统一固件）。固件文件一般有两种：
...
* [Linux_adb_fastboot](https://community.t-firefly.com/doc/download/217#other_615)工具

下载 [Linux_adb_fastboot](https://community.t-firefly.com/doc/download/217#other_615), 并按以下方法安装到系统中，方便调用：
...
1. 用金属镊子接通Station-P2上的如下图所示的两个测试点并保持(如下图所示)。
1. 设备插入电源。
1. 稍候片刻，之后松开镊子。
...
1. 设备断开电源
2. 用针状物捅住 Station-P2 上的 Recovery 孔 并保持
3. 设备插入电源上电
此时设备进入 Recovery 模式
