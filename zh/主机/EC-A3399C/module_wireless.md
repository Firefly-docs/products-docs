...
* 将模组的VCC、GND、TX、RX分别接到 EC-A3399C **UART?**(<font color = "red">对应节点为`/dev/tty*`</font>)的3.3V、GND、RX、TX，注意避免接错VCC、GND、TX、RX<font color = "red">导致烧坏模组</font>
...
* 使能GPS
    * 修改SDK目录下`device/rockchip/rk3399/.mk`(Android7.1以上的版本为`device/rockchip/rk3399//.mk`)里面`BOARD_HAS_GPS`参数为`true`，实现固件使能GPS功能，修改后重新编译SDK并烧录固件生效。

* 修改串口配置(串口节点或者波特率)
    * 将SDK目录下`device/rockchip/rk3399/gps/u-blox.conf`(Android7.1以上的版本为`device/rockchip/rk3399//gps/u-blox.conf`)里面`SERIAL_DEVICE`或者`SERIAL_BAUD_RATE`修改成对应模组配置的值，修改后重新编译SDK并烧录固件生效
