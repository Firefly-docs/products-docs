...
**注意**:GPS功能会占用到 **UART7**，若需要用 **UART7** 作为其他用途，需要将GPS先禁止掉
...
确保固件为最新[公版固件](https://www.t-firefly.com/doc/download/180.html#other_647)，固件默认关闭GPS功能，需要手动使能，以下是修改方法：
...
* 使能GPS
    * 修改SDK目录下`device/rockchip/rk356x/rk3568_firefly_roc_pc_se/rk3568_firefly_roc_pc_se.mk`里面`BOARD_HAS_GPS`参数为`true`，实现固件使能GPS功能，修改后重新编译SDK并烧录固件生效。

* 修改串口配置(串口节点或者波特率)
    * 将SDK目录下`device/rockchip/rk356x/rk3568_firefly_roc_pc_se/gps/u-blox.conf`里面`SERIAL_DEVICE`或者`SERIAL_BAUD_RATE`修改成对应模组配置的值，修改后重新编译SDK并烧录固件生效