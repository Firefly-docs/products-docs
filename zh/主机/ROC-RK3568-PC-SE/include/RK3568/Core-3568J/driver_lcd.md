...
ROC-RK3568-PC-SE的SDK下`kernel/arch/arm64/configs/firefly_defconfig`已经把LCD相关的配置设置好了，如果自己做了修改，请注意把以下配置加上：
```
...
ROC-RK3568-PC-SE的SDK有MIPI DSI的DTS文件：
`kernel/arch/arm64/boot/dts/rockchip/rk3568-firefly-aioj-mipi101_M101014_BE45_A1.dts`，此DTS文件为MIPI_DSI0+HDMI屏配置，MIPI_DSI1 默认关闭，接下来以此DTS中的DSI0配置作为范例来进行相关的LCD驱动介绍。
...
在硬件信号上LCD_EN引脚高电平有效，而LCD_RST引脚是低电平有效，具体的引脚配置请参考[《GPIO 使用》](driver_gpio.html)一节。需要注意的是ROC-RK3568-PC-SE的dsi1中，LCD_EN:(PCA_IO0_7)使用的是PCA9555扩展芯片引出来的gpio，需要PCA9555 驱动起来以后才能控制该gpio。
### 背光配置
...
```
mipi-data-delay-ms为10.1寸 M101014-BE45-A1 MIPI屏所需时序，调试屏幕时可根据本身上下电时序是否需要再配置此属性。MIPI上下电后需要发送初始化或退出指令才能使之正常工作，初始化和退出指令列表可以参考`kernel/arch/arm64/boot/dts/rockchip/rk3568-firefly-aioj-mipi101_CZNB101862T.dts`，列表如下：
```
&dsi0 {
	status = "okay";
	//rockchip,lane-rate = <1000>;
...
## EDP DTS配置
### 引脚配置
ROC-RK3568-PC-SE的SDK有EDP的DTS文件：`kernel/arch/arm64/boot/dts/rockchip/rk3568-firefly-aioj-edp_M156X40.dts`，从该文件中我们可以看到以下语句：
```
