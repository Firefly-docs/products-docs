...
本文介绍了如何将主机上的固件，通过Type-C 数据线烧录到 EC-A3588SJD4-AI 开发板的存储器中。升级时，需要根据主机操作系统和固件类型来选择合适的升级方式。

## 准备工具
* AIO-3588SJD4-AI 开发板
* [固件](https://community.t-firefly.com/doc/download/323)
* 主机
* 良好的Type-C 数据线

## 准备固件
固件可以通过编译SDK获得，也可以通过[资源下载](https://community.t-firefly.com/doc/download/323)处下载公版固件（统一固件）。固件文件一般有两种：
...
下载 [Release_DriverAssistant.zip](https://community.t-firefly.com/doc/download/323)，解压，然后运行里面的 DriverInstall.exe 。为了所有设备都使用更新的驱动，请先选择`驱动卸载`，然后再选择`驱动安装`。
<center>
...
### Linux操作系统
Linux 下无须安装设备驱动
* [Linux_Upgrade_Tool](https://community.t-firefly.com/doc/download/323)工具

下载 [Linux_Upgrade_Tool](https://community.t-firefly.com/doc/download/323), 并按以下方法安装到系统中，方便调用：
...
EC-A3588SJD4-AI 整机外壳上没有预留 recovery 按键，所以没有硬件进入 Loader 模式的办法。
#### 软件方式进入Loader模式
...
1. 设备断开电源
2. 用针状物捅住 EC-A3588SJD4-AI 上的 MaskRom 触点/孔位并保持
3. 设备插入电源上电
