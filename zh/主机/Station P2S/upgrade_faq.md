...
ROC-RK3568-PC-SE 板载 Recovery 按键与 MaskRom 测试点。若设备可以进入 Loader 模式，一般无需进入 MaskRom 模式；当 bootloader 损坏、设备无法进入 Loader 模式时，可按以下方法强行进入 MaskRom 模式：

* 先断开电源适配器连接，拆开外壳露出主板
* 用镊子或导线短接 MaskRom 测试点的 `GND` 与 `D0` 两个焊点，并保持短接
* 接上电源，大约两秒钟后，断开短接，此时设备进入 MaskRom 模式
