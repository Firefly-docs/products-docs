...
下载根文件系统：[Ubuntu 根文件系统](https://community.t-firefly.com/doc/download/229.html)，一般选择日期最新的文件系统。
...
修改文件```/path/to/yocto/meta-rockchip/conf/machine/firefly-rk3576.conf``` 中的 BB_NUMBER_THREADS 和 PARALLEL_MAKE 变量配置。若线程数量设置过大可能会导致机器内存不足，导致编译失败。请根据编译机器的配置来设置编译速度。
