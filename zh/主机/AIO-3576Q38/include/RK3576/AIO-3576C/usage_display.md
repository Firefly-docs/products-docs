...
AIO-3576Q38 有四种显示输出接口，分别是 HDMI、Display Port、 MIPI DSI 以及 LVDS，可以做到多屏同显/异显。
...
AIO-3576Q38 硬件上有一个 HDMI 显示输出接口：
...
AIO-3576Q38 有一个 Display Port 显示输出接口，支持  DP TX 1.4a 协议，分辨率最高可以支持 4k@120Hz。
...
AIO-3576Q38 有一路 MIPI DSI 显示输出接口，支持 DPHY2.0 和 4 Lane 的数据输出。  
...
AIO-3576Q38 有一路 LVDS 显示输出接口，它是由 TTL 转的 LVDS，支持最高输出 2K 分辨率。
...
AIO-3576Q38 硬件预留一个 eDP 显示输出接口 40Pin / 30Pin ( 默认硬件用作HDMI功能 )：
