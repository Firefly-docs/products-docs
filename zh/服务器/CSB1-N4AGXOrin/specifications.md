...
| **灯和按键** | - 1 个 UID 按键灯<br/>- 1 个 POWER 按键灯<br/>- 4 个计算节点系统指示灯<br/>- 2 个 SFP+ 指示灯<br/>- 1 个交换机指示灯<br/>- 1 个 BMC 系统指示灯 |
| **风扇** | 7 个冗余风扇 |
...
#### 硬件逻辑结构图
图5为服务器内部的硬件逻辑结构图，清晰展示了子板、BMC、存储和内部网络交换机等硬件连接关系。表6为图5中的符号说明：
| 符号 | 名称 | 说明 |
| :--- | :--- | :--- |
| nic0_5G | 以太网卡（速度 5 Gbps） | 支持 VLAN 划分 |
| Layer 3 switch | 内部三层交换机 | 支持 VLAN 划分、网络聚合和 QoS 等功能 |

![perspective view](../../../servers_img/CSB1-N4AGXOrin/hw_logical_topology.png)
...
| 标识 | 指示灯/按钮 | 状态说明 |
| :--- | :--- | :--- |
| 1-4 | 计算节点状态指示灯 | - 绿色（常亮）：表示计算节点已经正常开机。<br/>- 熄灭：表示计算节点未上电。 |
| | 电源按钮/指示灯 | **电源指示灯说明：**<br/>- 黄色（常亮）：表示服务器处于待机（Standby）状态。<br/>- 黄色（闪烁）：表示 BMC 管理系统正在启动。<br/>- 绿色（常亮）：表示服务器已经开机。<br/>- 熄灭：表示服务器未上电。<br/><br/>**电源按钮说明：**<br/>- 上电状态下短按该按钮，可以正常关闭子板 OS。<br/><br/>**详细关机过程：**<br/>- 第 1~5 秒：绿色（闪烁）——通知子计算单元需要在 5 秒内有序关闭业务程序。<br/>- 第 5~20 秒：黄绿（交替闪烁）——子计算单元正式开始关机。<br/>- 第 20 秒：黄色（常亮）——各个计算单元下电（除 BMC 外均下电）。<br/>- 待机状态下短按该按钮，可以进行上电。 |
...
| MGNT | RJ45 | - 用作 BMC 管理网络。<br/>- 1000/100/10 Mbps 自适应。 |
...
#### 硬盘位置
![perspective view](../../../servers_img/CSB1-N4AGXOrin/harddisk_location.png)
#### 硬盘配置
...
![SATA Hard Disk Indicator Location Diagram](../../../servers_img/CSB1-N4AGXOrin/sata_hdd_indicator.png)
