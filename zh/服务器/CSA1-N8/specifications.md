...
| 指标项 | 规格 |
| :--- | :--- |
| **尺寸（高×宽×深）** | 机箱：44.4 mm（1U）× 417.3 mm × 490 mm |
| **安装尺寸要求** | 可安装在满足 IEC 297 标准的通用机柜中：<br/>- 宽 19 英寸<br/>- 深 800 mm 及以上<br/><br/>滑道的安装要求如下：<br/>- 可伸缩滑道：机柜前后方孔条的距离范围为 543.5 mm ～ 848.5 mm |
| **满配重量** | - 净重：6.7 kg<br/>- 包装材料重量：8.9 kg |
| **能耗** | 整机因搭载的计算单元数量和种类不同，能耗存在差异。以下为参考值：<br/>- 典型能耗：300 W<br/>- 最大能耗：450 W |
...
| 符号 | 名称 | 说明 |
| :--- | :--- | :--- |
| Layer 2 switch | 内部二层交换机 | 不支持 VLAN 划分、网络聚合和 QoS 等功能 |

![perspective view](../../../servers_img/CSA1-N8/hw_logical_topology.png)

#### 网络拓扑图
根据硬件结构逻辑图可知，阵列式服务器中集成的ARM核心板与BMC是通过两个二层交换机实现网络互联的，

![perspective view](../../../servers_img/CSA1-N8/default_network_topology.png)
...
| 标识 | 指示灯/按钮 | 状态说明 |
| :--- | :--- | :--- |
| 1-8,M | 计算节点状态指示灯 | - 绿色（常亮）：表示计算节点已经正常开机。<br/>- 熄灭：表示计算节点未上电。 |
| 1-8,M | 网络状态指示灯 | - 黄色（闪烁）：表示子板正在通信。<br/>- 熄灭：子板未通信。 |
...
| 标识 | 接口名称 | 说明 |
| :--- | :--- | :--- |
| SFP+1 / SFP+2 | SFP+ 端口 | - 万兆光口默认速率为 10 Gbps 自适应 |
...
| 标识 | 接口名称 | 说明 |
| :--- | :--- | :--- |
| HIDMI | HIDMI 接口 | - BMC 直出 HIDMI <br/>- 分辨率为 1080P。 |
